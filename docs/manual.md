> 本文件是 KVM Manager 完整手册，由项目根 README 迁移而来，保留全部部署步骤、使用说明、API 清单与版本历史；新版精简说明见根目录 [README.md](../README.md)。
...
| 文档 | 适用场景 |
| :-: | :-: |
| [docs/frontend-refresh-functions.md](./frontend-refresh-functions.md) | 查看自动刷新、手动刷新、SSE 事件、fast/full 边界和各页面刷新入口 |
| [docs/vm-info-collection.md](./vm-info-collection.md) | 查看虚拟机字段来源、采集命令、CPU/内存/磁盘/I/O 计算口径和回退策略 |
| [docs/host-info-collection.md](./host-info-collection.md) | 查看宿主机字段来源、资源使用率、宿主机趋势和接口采集链路 |
| [docs/operation-log-coverage.md](./operation-log-coverage.md) | 查看任务、审计日志、告警、通知投递记录的覆盖范围和失败记录边界 |
| [docs/agent-command-timeout-and-temp-dirs.md](./agent-command-timeout-and-temp-dirs.md) | 排查 Agent 命令超时、`COMMAND_TIMEOUT_SECONDS` 作用范围和 `/tmp` 临时目录来源 |
| [docs/network-interface-dns-bridge-implementation.md](./network-interface-dns-bridge-implementation.md) | 查看宿主机接口 DNS 写入、NAT/ROUTE/BRIDGE 网络池校验和桥接能力边界 |
| [docs/frontend-select-dropdown-placement.md](./frontend-select-dropdown-placement.md) | 查看前端 Select/listbox 下拉展开方向、弹窗内下拉遮挡和控件维护规范 |
...
- 宿主机字段来源、采集命令和趋势指标口径详见 [docs/host-info-collection.md](./host-info-collection.md)。
...
- 虚拟机字段来源、采集命令和 CPU/内存/磁盘/I/O 计算口径详见 [docs/vm-info-collection.md](./vm-info-collection.md)。
...
- 各页面按钮的刷新范围不同，例如单台 VM、快照、存储池、网络池、接口、监控曲线或验证码；详情见 `./frontend-refresh-functions.md`。
- 若需要排查 Agent 外部命令耗时、`COMMAND_TIMEOUT_SECONDS` 和宿主机 `/tmp` 下 `go-build*`、`libguestfs*` 临时目录来源，详见 [docs/agent-command-timeout-and-temp-dirs.md](./agent-command-timeout-and-temp-dirs.md)。
...
- 任务、审计和告警的详细边界与操作覆盖矩阵详见 `./operation-log-coverage.md`。
...
| 企业微信扫码有效期 | 企业微信授权 state 的有效窗口，超时需重新扫码登录或绑定；范围 1-60 分钟，默认 5 分钟 |
...
- 配置保存后可直接发送测试通知。
- 前端 Select/listbox 类控件的下拉方向、弹窗内遮挡处理和维护清单详见 [docs/frontend-select-dropdown-placement.md](./frontend-select-dropdown-placement.md)。
...
- 后端定时任务会创建全局运行态轻量刷新任务，手动 `/api/refresh` 会创建 full 全量刷新任务；任务完成并收到 `runtime.updated` 后，页面展示的是刷新任务写入缓存后的最新运行态。刷新类型和页面刷新范围详见 `./frontend-refresh-functions.md`。
...
- 定时 fast 刷新由 `RUNTIME_SYNC_INTERVAL` 控制，默认 30 秒。
- 到达间隔后，后端创建或复用 `runtime.refresh.fast` 任务，更新 Redis 运行态缓存，并通过 SSE 通知页面重新读取数据。
- 如果当前没有任何已登记 Agent，定时 fast 刷新不会创建任务。
- fast 会更新 VM 内存使用率等指标样本，但不会执行 Guest Agent OS/IP 查询、磁盘明细和快照采集。
- 低频 deep 刷新由 `RUNTIME_DEEP_SYNC_INTERVAL` 控制，默认 10 分钟，会创建或复用 `runtime.refresh.all` 任务补采 IP、操作系统、内存、磁盘和快照等较重详情。
- 无已登记 Agent 时，低频 deep 刷新也不会创建任务。
- 右上角全量刷新图标或手动 `/api/refresh` 也是 `runtime.refresh.all` full 全量刷新。
- 快照页的 `POST /api/snapshots/refresh` 只刷新快照列表。

详情见 `./frontend-refresh-functions.md`。

## 8.2 前端点击刷新拿到的是最新数据吗？

不同刷新入口的范围不同：

- 右上角全量刷新图标会触发 `POST /api/refresh`，刷新所有 Agent 的完整宿主机、VM 详情和快照缓存。
- 单台 VM 刷新只同步该 VM 的最新运行态。
- 快照页刷新只刷新快照缓存。
- 存储池、网络池和接口页刷新只重新读取当前宿主机对应资源。
- 监控刷新只重新读取指标曲线。
- 快照恢复和 VM 配置类操作会自动定向刷新目标 VM。
- 存储池、网络池和接口变更后，前端对应资源页会自动刷新。

各入口是否触发 Agent 采集、刷新范围和接口详见 `./frontend-refresh-functions.md`。
...
- KVM/libvirt 中的 `qcow2`、`raw` 主要表示磁盘文件格式。
- VMware 的精简置备、厚置备延迟置零、厚置备置零更偏向空间分配策略。
- 当前项目中 `qcow2` 默认按需增长，更接近精简置备。
- 创建虚拟机和添加镜像时 metadata 预分配默认关闭。
- 勾选 `preallocMetadata` 后只预分配元数据，不等同于厚置备。
- `raw` 是否表现为精简或厚置备，取决于创建命令、文件系统和后端存储能力。

## 8.7 创建虚拟机时磁盘卷名已存在会怎样？

处理规则如下：

- 后端会在创建任务排队前检查目标存储池中的卷名。
- 发现同名卷时，后端直接拒绝创建，并提示更换磁盘卷名称。
- 如果并发场景下预检后又出现同名卷，Agent 执行 `virsh vol-create-as` 时也会失败。
- Agent 不会覆盖已有卷。
- 如果本次新卷已成功创建，但后续步骤失败，会在失败清理流程中删除该新卷。

## 8.8 创建虚拟机多块磁盘必须使用同一种总线吗？

需要保持一致。

- 创建弹窗中添加数据盘时，数据盘的存储池、磁盘格式、磁盘总线、卷名称和 `preallocMetadata` 会继承系统盘配置并禁用编辑。
- 数据盘只允许单独填写容量。
- 后端和 Agent 会校验数据盘配置必须与系统盘一致。
- 实际使用中建议默认选择 `virtio`，以获得较好性能。
- Windows 虚拟机如果安装阶段缺少 VirtIO 驱动，可考虑系统盘先使用 `sata`。
- 数据盘会随系统盘保持相同总线。
...
验证邮箱用于确认当前操作者确实知道该用户配置的邮箱，避免只凭公开用户名触发重置流程。

- 找回密码验证码仅通过邮件媒介发送到账号配置邮箱。
- 验证码不会发送到邮件通知配置中的告警收件人列表。
- AD/LDAP 用户不走本地找回密码流程，需要在目录服务侧重置密码。

## 8.11 通知媒介的“告警通知”和“找回密码”开关有什么区别？

三个开关的用途不同：

- 告警通知开关决定活跃告警是否推送到该外部媒介。
- 恢复通知开关决定告警解决时是否发送恢复内容，且仅在告警通知开启时生效。
- 找回密码开关仅在邮件媒介中显示，决定邮件媒介是否出现在忘记密码流程的发送选项中。
...
如果外部媒介响应较慢或频繁失败，可继续检查基础配置中的通知策略：

- 发送超时是否过短。
- 最大重试次数是否过低。
- 单轮处理批量是否过小。

未配置外部告警媒介时，右上角通知中心的站内通知视为已触达，系统不会因为缺少外部媒介而反复重试同一活跃告警。
...
任务、审计和告警分别用于记录后台任务进度、用户关键操作和平台运行态异常；具体覆盖范围、失败记录规则和后续开发同步要求详见 `./operation-log-coverage.md`。
...
- `POST /api/auth/login`  - 登录，返回访问令牌（JWT）与用户信息；会话有效期由 `JWT_EXPIRE_HOURS` 决定，签发时固定到期时间；同一账号连续密码失败达到「安全时效」配置的锁定次数后返回 429 并提示剩余等待分钟数，锁定时长内即使密码正确也会被拒绝，登录成功或锁定窗口过期后重新计数，内置管理员 admin 不受限
...
- `POST /api/host-interfaces/{agentId}` - 在指定宿主机 Agent 上创建 Linux bridge 接口
  ...
  - 需要宿主机接口管理权限
  - DNS 写入、ifcfg 备份和桥接能力边界详见 [docs/network-interface-dns-bridge-implementation.md](./network-interface-dns-bridge-implementation.md)
...
- `POST /api/network-pools/{agentId}` - 在指定宿主机 Agent 上创建网络池，支持 NAT、ROUTE、ISOLATE 和 BRIDGE；NAT/ROUTE 创建前检查 IPv4 转发，BRIDGE 创建前检查桥接设备存在
  - NAT/ROUTE/BRIDGE 创建前校验和桥接能力边界详见 [docs/network-interface-dns-bridge-implementation.md](./network-interface-dns-bridge-implementation.md)
...
- 直连模式通过一次性 state（取出即删，防重放，有效期为基础配置中的企业微信扫码有效期，默认 5 分钟）区分登录与绑定用途；code 换取身份时使用带缓存的 access_token（到期前 5 分钟刷新，失效自动重试一次）。
- 直连模式的回调地址优先取认证配置中的外部访问地址，留空时按用户当前访问地址（含反向代理协议头）自动推断。
- 统一认证中心模式的 verify 请求由后端发起并携带 HMAC-SHA256 签名，`appSecret` 不进前端；票据一次性，校验失败需重新扫码。
- 登录成功后通过 URL fragment 将会话 Token 回传前端 `/auth/callback` 页面，Token 不进服务端日志与 Referer；地址栏中的授权结果会被立即清除，防止刷新重复消费；失败时回传中文错误提示。
- 企业微信登录/绑定失败会写入审计日志（`auth.wecom.failed`），包含失败原因与企微账号；绑定成功写 `auth.wecom.bind`，解绑写 `auth.wecom.unbind`。
...
- 新增 `LOG_RETENTION_DAYS` 后端环境变量，统一控制任务、审计和告警日志按 `created_at` 创建时间保留。
- 后端新增日志保留清理协程，默认保留 30 天，设置为 `0` 可关闭自动清理。
- 前端格式化脚本补充 `tsx` 文件范围，并对 React 页面和组件执行统一格式化。
- 同步更新环境变量模板、日志覆盖说明、项目结构和部署示例文档。

详细更新日志见 [verchanglog/v1.1.6.md](../verchanglog/v1.1.6.md)。

## v1.1.5

- 优化存储池列表页卷数量加载，避免进入页面时逐卷执行格式探测。
- 存储池列表页和详情页均按 libvirt `Type` 排除 `dir` 目录条目，避免目录被展示为可操作卷。
- 修复旧版 virsh 不支持 `vol-list --name` 时卷数量显示为 `0` 的问题。
- 同步更新刷新说明、虚拟机采集说明和 Agent 存储池相关测试。

详细更新日志见 [verchanglog/v1.1.5.md](../verchanglog/v1.1.5.md)。
...
- 新增 Agent 删除短期 Redis 标记，并在运行态写入前后复查删除状态，避免并发刷新导致缓存残留。
- 宿主机、虚拟机和总览读取链路会过滤并清理数据库中已不存在的 Agent 孤儿缓存。
- 无已登记 Agent 时，后端定时 fast 刷新和低频 deep 刷新不再创建空任务。
- 优化 Agent 保存、同步和删除 toast 反馈，保存同步卡片固定展示并可复制内容。

详细更新日志见 [verchanglog/v1.1.3.md](../verchanglog/v1.1.3.md)。

## v1.1.2

- 优化新增 Agent 保存体验，保存成功后立即释放按钮，并在后台执行首次 full 同步。
- 总览页虚拟机运行 / 总数、状态分布、vCPU 已分配和最近虚拟机列表统一排除已标记模板。
- 模板标记或取消标记后，总览会通过 `runtime.updated` 重新读取汇总并使用最新非模板统计口径。
- 同步更新 README、Swagger/OpenAPI 文档、刷新说明、宿主机采集说明和总览统计测试。

详细更新日志见 [verchanglog/v1.1.2.md](../verchanglog/v1.1.2.md)。
