...
后端通过 `LOG_RETENTION_DAYS` 统一控制任务、审计和告警日志保留天数，默认 `30` 天。

- `tasks` 按 `created_at` 清理超过保留期的任务记录。
- `audit_logs` 按 `created_at` 清理超过保留期的审计记录。
- `alerts` 按 `created_at` 清理超过保留期的告警记录。
- `LOG_RETENTION_DAYS <= 0` 时关闭任务、审计和告警日志自动清理。
...
| 企业微信登录失败 | 否 | 是 | 否 | 写入 `auth.wecom.failed`，metadata 携带 `reason`（`state_invalid`/`provider_disabled`/`user_not_provisioned`/`wecom_api_failed`）；用户未开通时附带 `userid`（企微账号），不含 token、code 或其他敏感信息 |
