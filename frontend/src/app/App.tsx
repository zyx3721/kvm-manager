...
import { clearSession, getStoredUser, isAuthenticated, isSessionIdleExpired, markSessionActivity, userHasAnyPermission } from "../lib/auth";
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex h-screen items-center justify-center text-sm" style={{ color: "var(--kvm-text-muted)", background: "var(--kvm-bg)" }}>
          页面出现错误，请刷新重试
        </div>
      );
    }
...
function RequireAuth() {
  ...
  if (permissions && !userHasAnyPermission(user, permissions)) {
    const fallback = Object.entries(routePermissions).find(([, required]) => userHasAnyPermission(user, required))?.[0];
