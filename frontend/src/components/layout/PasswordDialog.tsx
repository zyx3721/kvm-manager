...
export default function PasswordDialog({ open, onClose, onSuccess }: PasswordDialogProps) {
  ...
  return (
    <DialogPortal>
    <div className="kvm-dialog-backdrop fixed inset-0 z-[70] flex items-center justify-center px-4" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && closeDialog()}>
      <form
      ...
      >
        <div className="mb-5 flex items-start gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg" style={{ background: "rgba(59,130,246,0.14)", border: "1px solid rgba(59,130,246,0.28)", color: "#60a5fa" }}>
            <KeyRoundIcon size={21} />
          </div>
          <div className="min-w-0 flex-1">
            <h2 id="change-password-title" className="text-base font-semibold" style={{ color: "var(--kvm-text)" }}>修改密码</h2>
            <p className="mt-1 text-sm" style={{ color: "var(--kvm-text-muted)" }}>输入旧密码后设置新的登录密码。</p>
          </div>
          <button type="button" className="kvm-action-button flex h-8 w-8 items-center justify-center rounded-lg" onClick={closeDialog} aria-label="关闭" style={{ color: "var(--kvm-text-muted)", background: "rgba(255,255,255,0.04)" }}>
            <XIcon size={16} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          {([
            ...
            <label key={field} className="block">
              <span className="mb-1.5 block text-xs font-medium" style={{ color: errors[field] ? "#f87171" : "var(--kvm-text-muted)" }}>{label}</span>
              <div className="relative">
                <input
                  ...
                  style={{ background: "rgba(255,255,255,0.04)", borderColor: errors[field] ? "rgba(248,113,113,0.7)" : "var(--kvm-border)", color: "var(--kvm-text)" }}
                />
                <button
                  type="button"
        ...
        {message && <div className="mt-4 rounded-lg px-3 py-2 text-sm" style={{ color: "#fca5a5", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)" }}>{message}</div>}

        <div className="mt-6 flex justify-end gap-3">
          <button type="button" className="kvm-action-button rounded-lg px-4 py-2 text-sm" onClick={closeDialog} style={{ color: "var(--kvm-text-muted)", background: "rgba(255,255,255,0.04)", border: "1px solid var(--kvm-border)" }}>
            取消
          </button>
          <button type="submit" disabled={submitting} className="kvm-action-button flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium disabled:opacity-60" style={{ color: "#ffffff", background: "linear-gradient(135deg, #2563eb, #06b6d4)", border: "1px solid rgba(96,165,250,0.35)" }}>
