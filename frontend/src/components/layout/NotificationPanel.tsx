...
const NotificationPanel = forwardRef<HTMLDivElement, NotificationPanelProps>(function NotificationPanel(
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 text-xs">
          <button type="button" onClick={onReadAll} className="kvm-action-button" style={{ color: 'var(--kvm-accent-text)' }}>
            全部已读
          </button>
          <button type="button" onClick={onClear} className="kvm-action-button" style={{ color: '#f87171' }}>
            清空
          </button>
          <button type="button" onClick={onClose} className="kvm-action-button md:hidden" style={{ color: 'var(--kvm-text-muted)' }}>
            关闭
          </button>
        </div>
      </div>
      <div className="kvm-hidden-scrollbar max-h-[28rem] overflow-y-auto">
        {items.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm" style={{ color: 'var(--kvm-text-muted)' }}>
            没有更多了
          </div>
        ) : (
          items.map(item => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  {!item.readAt && <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: '#ef4444' }} />}
                  <span className="truncate text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>
                    {item.title}
                  </span>
                  <span className="ml-auto shrink-0 text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
                    {formatTimeAgo(item.lastSeenAt)}
                  </span>
                </div>
                <div className="mt-1 line-clamp-2 text-sm leading-5" style={{ color: 'var(--kvm-text-muted)' }}>
                  {item.message}
                </div>
              </div>
            </button>
