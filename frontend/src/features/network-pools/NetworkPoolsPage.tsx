  ...
  return (
    <div data-cmp="NetworkPoolsPage" className="space-y-5 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold" style={{ color: 'var(--kvm-text)' }}>网络池</h1>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        ...
        <MetricCard icon={RouterIcon} label="桥接网络" value={`${items.filter(item => item.forward === 'bridge').length}`} />
        <MetricCard icon={Globe2Icon} label="DHCP" value={`${items.filter(item => item.dhcp).length}`} />
      </section>

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {!loading && items.length === 0 && <Empty text={host ? '当前宿主机暂无网络池' : '暂无可用宿主机'} />}
        {items.map(item => <NetworkPoolCard key={item.name} item={item} onDetail={item => void openDetail(item)} />)}
      </section>
      ...
      {detailItem && selectedHost && (
        <NetworkPoolDetailDialog
          ...
          onUpdateState={async active => updateNetworkPoolState(selectedHost, detailItem.name, active)}
          onUpdateAutostart={async autostart => updateNetworkPoolAutostart(selectedHost, detailItem.name, autostart)}
          onDeleted={async () => {
            await load(selectedHost);
          }}
        />
...
function NetworkPoolCard({ item, onDetail }: { item: NetworkPool; onDetail: (item: NetworkPool) => void }) {
  return (
    <article className="kvm-surface-3d rounded-xl p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs uppercase" style={{ color: 'var(--kvm-text-muted)' }}>{item.state || 'unknown'} · {item.autostart ? 'autostart' : 'manual'}</p>
        </div>
      ...
      <div className="kvm-dialog-panel max-h-[82vh] w-[min(92vw,800px)] overflow-hidden rounded-2xl">
        <header className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--kvm-border)' }}>
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs" style={{ color: 'var(--kvm-text-muted)' }}>{hostName} · {(localItem.forward || 'isolate').toUpperCase()}</p>
          </div>
        ...
        <div className="kvm-hidden-scrollbar max-h-[calc(82vh-74px)] overflow-y-auto p-5">
          <section className="kvm-dialog-card rounded-xl p-4">
            <div className="kvm-detail-tile-grid grid gap-3 md:grid-cols-2">
              ...
              <InfoTile label="DHCP 范围" value={localItem.dhcpStart && localItem.dhcpEnd ? `${localItem.dhcpStart} - ${localItem.dhcpEnd}` : '-'} />
              <ValueTile label="Open vSwitch" value={localItem.openVSwitch ? '启用' : '关闭'} enabled={localItem.openVSwitch} />
              <ValueTile label="DHCP" value={localItem.dhcp ? '启用' : '关闭'} enabled={localItem.dhcp} />
              <ActionTile
                label="状态"
                ...
                extra={<DeletePoolButton active={active} disabled={busy !== ''} onClick={() => setConfirmDelete(true)} />}
              />
              <ActionTile
                label="随物理机同启"
          ...
          {(localItem.fixedAddresses || []).length > 0 && <FixedAddressTable items={localItem.fixedAddresses || []} />}
        </div>
      </div>
      {confirmDelete && (
        <ConfirmDialog
...
function ValueTile({ label, value, enabled }: { label: string; value: string; enabled: boolean }) {
  ...
  return <div className="rounded-lg border p-3" style={{ borderColor: color.border, background: color.background }}><div className="text-xs" style={{ color: 'var(--kvm-text-muted)' }}>{label}</div><div className="mt-2"><span className="inline-flex h-7 items-center rounded-full border px-2.5 text-xs font-semibold" style={{ background: color.buttonBg, borderColor: color.border, color: color.text }}>{value}</span></div></div>;
}

function ActionTile({ label, value, buttonLabel, tone, disabled, canManage, onClick, extra }: { label: string; value: ReactNode; buttonLabel: string; tone: 'primary' | 'success' | 'warning' | 'danger'; disabled: boolean; canManage: boolean; onClick: () => void; extra?: ReactNode }) {
  const color = actionColor(tone);
  return <div className="rounded-lg border p-3" style={{ borderColor: color.border, background: color.background }}><div className="text-xs" style={{ color: 'var(--kvm-text-muted)' }}>{label}</div><div className="mt-2 flex items-center justify-between gap-3">{value}{canManage && <div className="flex shrink-0 items-center gap-2"><button type="button" disabled={disabled} onClick={onClick} className="kvm-action-button h-8 rounded-lg border px-3 text-xs font-semibold disabled:opacity-60" style={{ borderColor: color.border, background: color.buttonBg, color: color.text }}>{buttonLabel}</button>{extra}</div>}</div></div>;
}

function DeletePoolButton({ active, disabled, onClick }: { active: boolean; disabled: boolean; onClick: () => void }) {
  const button = <button type="button" disabled={active || disabled} onClick={onClick} className="kvm-action-button kvm-danger-button inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50" style={{ borderColor: 'rgba(239,68,68,0.35)', color: '#f87171', background: 'rgba(239,68,68,0.1)' }}><Trash2Icon size={13} />删除</button>;
  if (!active) return button;
  return <KvmTooltip label="运行中的网络池需要先停止后才能删除" placement="top">{button}</KvmTooltip>;
}

function FixedAddressTable({ items }: { items: NetworkFixedAddress[] }) {
  const [query, setQuery] = useState('');
  const filtered = items.filter(item => `${item.address} ${item.mac}`.toLowerCase().includes(query.trim().toLowerCase()));
  return (
    <section className="mt-4 rounded-xl border p-4" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-card)' }}>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-base font-semibold" style={{ color: 'var(--kvm-text)' }}>固定地址</h3>
        <div className="flex items-center gap-2">
          <input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索 IP 或 MAC" className="h-9 w-48 rounded-lg px-3 text-sm outline-none" style={fieldStyle} />
          <button type="button" onClick={() => setQuery('')} className="kvm-action-button h-9 rounded-lg border px-3 text-sm" style={buttonStyle}>清空</button>
        </div>
      </div>
      <div className="overflow-hidden rounded-lg border" style={{ borderColor: 'var(--kvm-border)' }}>
        <div className="kvm-hidden-scrollbar max-h-56 overflow-y-auto">
          <table className="w-full min-w-[420px] text-sm">
            <thead className="sticky top-0 z-10" style={{ background: 'var(--kvm-table-head-bg)', color: 'var(--kvm-text)' }}>
              <tr>
            ...
            <tbody style={{ color: 'var(--kvm-text-muted)' }}>
              {filtered.length === 0 && <tr><td colSpan={2} className="px-3 py-6 text-center">暂无匹配地址</td></tr>}
              {filtered.map(item => (
                <tr key={`${item.address}-${item.mac}`} style={{ borderTop: '1px solid var(--kvm-border)' }}>
                  <td className="px-3 py-3 text-center font-mono" style={{ color: 'var(--kvm-text)' }}>{item.address}</td>
...
function NetworkPoolDialog({ hostName, existing, onClose, onSubmit }: { hostName: string; existing: NetworkPool[]; onClose: () => void; onSubmit: (payload: NetworkPoolCreatePayload) => Promise<void> }) {
  ...
  const payload = { name, subnet, dhcp, fixedAddress, type: type.toLowerCase(), bridge, openVSwitch };
  ...
  return (
    ...
    <div className="kvm-dialog-backdrop fixed inset-0 z-50 flex items-center justify-center px-3">
      <div className="kvm-dialog-panel w-[min(92vw,620px)] overflow-hidden rounded-2xl">
        <header className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--kvm-border)' }}>
          <div>
            <h2 className="text-base font-semibold" style={{ color: 'var(--kvm-text)' }}>新增网络池</h2>
            <p className="mt-1 text-xs" style={{ color: 'var(--kvm-text-muted)' }}>{hostName}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="关闭" className="kvm-action-button inline-flex h-8 w-8 items-center justify-center rounded-lg border" style={buttonStyle}><XIcon size={15} /></button>
        </header>
        <div className="mx-auto max-w-[500px] space-y-4 p-5">
          <Field label="名称"><input value={name} onChange={event => setName(event.target.value)} placeholder="default" className="h-10 w-full rounded-lg px-3 text-sm outline-none" style={fieldStyle} /></Field>
          <Field label="网络类型">
            <SelectMenu
              value={type}
              placeholder="请选择网络类型"
          ...
          {!bridgeMode && <>
            <Field label="子网池"><input value={subnet} onChange={event => setSubnet(event.target.value)} placeholder="192.168.100.0/24" className="h-10 w-full rounded-lg px-3 text-sm outline-none" style={fieldStyle} /></Field>
            <Field label="DHCP"><RoundedCheckbox checked={dhcp} label="启用 DHCP" onChange={setDhcp} /></Field>
            <Field label={<FixedAddressLabel />}><RoundedCheckbox checked={fixedAddress} label="启用 Fixed Address" onChange={setFixedAddress} /></Field>
          </>}
          {bridgeMode && <>
            <Field label="桥接名称"><input value={bridge} onChange={event => setBridge(event.target.value)} placeholder="br0" className="h-10 w-full rounded-lg px-3 text-sm outline-none" style={fieldStyle} /></Field>
            <Field label={<OpenVSwitchLabel />}><RoundedCheckbox checked={openVSwitch} label="启用 Open vSwitch" onChange={setOpenVSwitch} /></Field>
          </>}
        </div>
        <footer className="flex justify-end gap-2 border-t px-5 py-4" style={{ borderColor: 'var(--kvm-border)' }}>
          <button type="button" onClick={onClose} className="kvm-action-button rounded-lg border px-4 py-2 text-sm" style={buttonStyle}>关闭</button>
          <button type="button" onClick={() => void onSubmit(payload)} className="kvm-action-button rounded-lg border px-4 py-2 text-sm font-semibold" style={primaryButtonStyle}>创建</button>
        </footer>
      </div>
    </div>
    </DialogPortal>
...
function RoundedCheckbox({ checked, label, onChange }: { checked: boolean; label: string; onChange: (checked: boolean) => void }) {
  return (
    <label className="inline-flex min-h-10 cursor-pointer items-center gap-2.5">
      ...
      <span
        ...
        style={{
          ...
          boxShadow: checked ? 'inset 0 1px 0 rgba(255,255,255,0.2), 0 0 0 2px rgba(45,212,191,0.08)' : 'inset 0 1px 0 rgba(255,255,255,0.08)',
          color: checked ? '#5eead4' : 'transparent',
        }}
      >
        <CheckIcon size={14} strokeWidth={3} />
      </span>
      <span className="text-sm" style={{ color: checked ? 'var(--kvm-text)' : 'var(--kvm-text-muted)' }}>{label}</span>
    </label>
  );
}
...
function validateNetworkPayload(payload: NetworkPoolCreatePayload, existing: NetworkPool[]) {
  ...
  if (existing.some(item => item.name.toLowerCase() === payload.name.trim().toLowerCase())) return '名称已存在，请重新更换名称';
  if (type === 'bridge') {
  ...
  if (existing.some(item => sameSubnet(item.subnet, payload.subnet))) return '子网池已存在，请重新更换子网';
  return '';
}
...
function isCidr(value?: string) {
  ...
  return value.split('/')[0].split('.').every(part => Number(part) >= 0 && Number(part) <= 255);
}
...
function Field({ label, children }: { label: ReactNode; children: ReactNode }) {
  return <label className="grid gap-2 md:grid-cols-[100px_minmax(0,340px)] md:items-center md:justify-center"><span className="text-sm font-semibold md:text-right" style={{ color: 'var(--kvm-text)' }}>{label}</span>{children}</label>;
}
...
function friendlyPoolError(error: unknown) {
  ...
  const compact = raw.replace(/\s+/g, ' ').trim().replace(/^创建网络池失败[:：]\s*/, '');
  const lower = compact.toLowerCase();
  if (lower.includes('subnet is too small for dhcp')) return '子网可用地址不足，无法启用 DHCP';
  if (lower.includes('subnet is too small for fixed address')) return '子网可用地址不足，无法生成固定地址';
  if (lower.includes('fixed address range is too large')) return '固定地址范围过大，请缩小子网或关闭固定地址';
  if (lower.includes('bridge device does not exist')) return '桥接设备不存在，请先在接口页创建或选择已有 bridge';
  ...
  if (lower.includes('ip forwarding sysctl is unavailable')) return '宿主机 IP 转发配置不可用，请检查 KVM 网络环境';
  ...
  if (lower.includes('virsh') || lower.includes('error:')) return '宿主机命令执行失败，请检查名称、子网和权限';
