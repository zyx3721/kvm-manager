    ...
    <div data-cmp="HostInterfacesPage" className="space-y-5 p-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {!loading && items.length === 0 && (
          <Empty text={host ? '当前宿主机暂无接口' : '暂无可用宿主机'} />
        )}
        {items.map(item => (
          <HostInterfaceCard key={item.name} item={item} onDetail={item => void openDetail(item)} />
        ))}
      </section>
          ...
          onSubmit={async payload => {
            ...
            try {
              ...
              if (isAgentTimeoutError(error)) {
                const latest = await load(selectedHost);
                if (
                  latest.some(item => item.name.toLowerCase() === payload.name.trim().toLowerCase())
                ) {
                  toast.success('接口已创建');
                  setDialogOpen(false);
                  return;
...
function HostInterfaceCard({
  item,
  onDetail,
}: {
  item: HostInterface;
  onDetail: (item: HostInterface) => void;
}) {
          ...
          <div className="kvm-hidden-scrollbar max-h-[calc(82vh-74px)] overflow-y-auto p-5">
            <section className="kvm-dialog-card rounded-xl p-4">
              <div className="kvm-detail-tile-grid grid gap-3 md:grid-cols-2">
                ...
                <InfoTile
                  label="IPv4"
                  value={formatAddressMode(localItem.ipv4, localItem.ipv4Mode)}
                />
                <InfoTile
                  label="IPv6"
                  value={formatAddressMode(localItem.ipv6, localItem.ipv6Mode)}
                />
...
function validatePayload(payload: HostInterfaceCreatePayload, existing: HostInterface[]) {
  ...
  if (!/^[A-Za-z0-9_.-]{1,15}$/.test(payload.name.trim()))
    return '接口名称只能包含字母、数字、点、短横线和下划线，长度不超过 15';
  if (!['bridge', 'ethernet'].includes(payload.type))
    return '当前仅支持创建 bridge 或 ethernet 类型接口';
  if (
    payload.type === 'bridge' &&
    (Number.isNaN(Number(payload.delay)) || Number(payload.delay) < 0)
  )
    return 'Delay 必须是大于等于 0 的数字';
...
function friendlyInterfaceError(error: unknown) {
  ...
  if (lower.includes('ipv4 address already exists'))
    return 'IPv4 地址已被其他接口使用，请更换后重试';
  if (lower.includes('ipv6 address already exists'))
    return 'IPv6 地址已被其他接口使用，请更换后重试';
  if (lower.includes('ipv4 subnet already exists'))
    return 'IPv4 子网已被其他接口使用，请避免同宿主机重复子网';
  if (lower.includes('ipv6 subnet already exists'))
    return 'IPv6 子网已被其他接口使用，请避免同宿主机重复子网';
  if (lower.includes('ipv4 gateway must be in the same subnet'))
    return 'IPv4 网关必须与地址处于同一子网';
  if (lower.includes('ipv6 gateway must be in the same subnet'))
    return 'IPv6 网关必须与地址处于同一子网';
  ...
  if (lower.includes('interface device already in use'))
    return '该设备已被其他接口使用，请重新选择设备';
  if (lower.includes('system interface config not found'))
    return '未找到宿主机系统网络配置，无法写入 DNS';
  if (
    lower.includes('write system interface dns failed') ||
    lower.includes('backup system interface config failed')
  )
    return '写入宿主机 DNS 配置失败，请检查 Agent 权限';
  if (lower.includes('already exists')) return '接口名称已存在，请重新更换名称';
  if (lower.includes('interface not found') || lower.includes('not found'))
    return '接口不存在或已被删除，请刷新后重试';
  ...
  if (lower.includes('start mode is invalid'))
    return '启动模式不受支持，请选择 none、onboot 或 hotplug';
  if (lower.includes('unsupported interface type') || lower.includes('unsupported'))
    return '当前接口类型或参数不受支持';
  ...
  if (lower.includes('permission denied') || lower.includes('operation not permitted'))
    return '宿主机权限不足，无法创建接口';
  if (
    lower.includes('iface-define') ||
    lower.includes('define interface') ||
    lower.includes('failed to define')
  )
    return '接口定义失败，请检查接口类型、绑定设备和地址配置';
  if (
    lower.includes('iface-start') ||
    lower.includes('start interface') ||
    lower.includes('failed to start')
  )
    return '接口启动失败，请检查启动模式、绑定设备和宿主机网络配置';
  if (
    lower.includes('iface-destroy') ||
    lower.includes('destroy interface') ||
    lower.includes('failed to destroy')
  )
    return '接口停止失败，请检查接口当前状态和宿主机权限';
  if (
    lower.includes('iface-undefine') ||
    lower.includes('undefine interface') ||
    lower.includes('failed to undefine')
  )
    return '接口删除失败，请确认接口已停止且不再被使用';
  if (
    lower.includes('xml') ||
    lower.includes('invalid argument') ||
    lower.includes('unsupported configuration')
  )
    return '接口配置不被 libvirt 接受，请检查类型、设备和协议配置';
  if (lower.includes('ip link show')) return '读取绑定设备失败，请确认设备仍存在';
  if (lower.includes('ip ')) return '宿主机 ip 命令执行失败，请检查设备名称和权限';
  if (lower.includes('failed'))
    return `宿主机命令执行失败：${compact.length > 90 ? `${compact.slice(0, 90)}...` : compact}`;
...
function InterfaceActionTile({
  ...
  return (
    <div
      className="rounded-lg border p-3"
      style={{
        borderColor: active ? 'rgba(16,185,129,0.34)' : 'rgba(239,68,68,0.34)',
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        ...
        {canManage && (
          <div className="flex shrink-0 items-center gap-2">
            ...
            {active ? (
              <KvmTooltip label="运行中的接口需要先停止后才能删除" placement="top">
                {deleteButton}
              </KvmTooltip>
            ) : (
              deleteButton
...
function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/35 px-3">
      <div
        className="kvm-dialog-card w-[min(92vw,420px)] rounded-xl border p-5"
        style={{ borderColor: 'var(--kvm-border)' }}
      >
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button
            ...
            disabled={busy}
            onClick={onClose}
            className="kvm-action-button rounded-lg border px-4 py-2 text-sm disabled:opacity-60"
            style={buttonStyle}
          >
            取消
          </button>
          <button
            ...
            disabled={busy}
            onClick={onConfirm}
            className="kvm-action-button kvm-danger-button rounded-lg border px-4 py-2 text-sm font-semibold disabled:opacity-60"
            style={{
              borderColor: 'rgba(239,68,68,0.35)',
              color: '#f87171',
              background: 'rgba(239,68,68,0.1)',
  ...
  return (
    ...
    >
      ...
      <div className="mt-2">
        <span
          className="inline-flex h-7 items-center rounded-full border px-2.5 text-xs font-semibold"
          style={{ color, borderColor: active ? 'rgba(16,185,129,0.34)' : 'rgba(239,68,68,0.34)' }}
        >
          {value}
        </span>
      </div>
