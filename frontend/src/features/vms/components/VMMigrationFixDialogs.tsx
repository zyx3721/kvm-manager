...
export function MigrationSSHKeyDialog({
  vmId,
  vmName,
  payload,
  message,
  ...
  return (
    ...
    >
      ...
      >
        <FixMessage
          message={
            message || '源宿主机无法免密连接目标 libvirt，请输入目标宿主机 SSH 密码完成免密配置'
          }
          vmName={vmName}
        />
...
export function MigrationHostnameDialog({
  vmId,
  vmName,
  payload,
  message,
  ...
  return (
    ...
    >
      ...
      >
        <FixMessage
          message={
            message || '目标宿主机主机名解析为 localhost，请输入真实主机名并写入源目标 hosts 解析'
          }
          vmName={vmName}
        />
        ...
        <FixNotice>
          将设置目标宿主机 hostname，并在源宿主机和目标宿主机 /etc/hosts 写入目标 IP 与主机名解析
        </FixNotice>
      </div>
      <DialogActions label={busy ? '配置中' : '配置并重新预检'} disabled={busy} onClick={submit} />
    </MigrationFixShell>
...
function FixMessage({ message, vmName }: { message: string; vmName: string }) {
  return (
    <>
      <div className="mb-3 text-xs leading-5" style={{ color: 'var(--kvm-text-muted)' }}>
        {message}
      </div>
