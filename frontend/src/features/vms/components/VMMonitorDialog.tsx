...
import { fetchVMMetricSeries, type MetricPoint, type VirtualMachine } from '../../../lib/api';
import {
  buildTimeTicks,
  formatTimeTick,
  formatTooltipTime,
  getChartWindow,
  type TimeWindow,
} from '../../../components/kvm/metricTimeAxis';
...
const cards: MonitorCard[] = [
  ...
  },
  {
    ...
    unit: '%',
    domain: [0, 100],
    lines: [{ key: 'disk', name: '磁盘', color: '#34d399' }],
  },
  {
    title: '磁盘 I/O',
    unit: 'KB/s',
...
export function VMMonitorDialog({ vm, onClose }: { vm: VirtualMachine; onClose: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchVMMetricSeries(
        vm.id,
        range,
        range === 'custom'
  ...
  }, [customEnd, customStart, range, vm.id]);
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(value, name) => [`${value} ${card.unit}`, name]}
                contentStyle={{
                  background: 'var(--kvm-menu-bg)',
