...
export function ClonePanel({
    ...
    () =>
      config?.disks.length
        ...
        : vm.disks.length > 0
          ...
          : [{ name: 'disk', path: '', bytes: vm.diskBytes, pool: '', bus: '', device: 'disk', type: 'file' }],
  ...
  const sourceMemoryMB = Math.max(1, Math.round((config?.currentMemoryBytes || vm.memoryBytes) / 1024 / 1024));
  const sourceMaxMemoryMB = Math.max(sourceMemoryMB, Math.round((config?.maximumMemoryBytes || vm.memoryBytes) / 1024 / 1024));
  const hostCPU = config?.hostCpu || 0;
  const hostMemoryMB = config?.hostMemoryBytes ? Math.floor(config.hostMemoryBytes / 1024 / 1024) : 0;
  ...
  const [interfacePools, setInterfacePools] = useState<Record<string, string>>(() => Object.fromEntries(interfaces.map(item => [item.name, item.source])));
  const [metadata, setMetadata] = useState<Record<string, boolean>>({});
  const [diskNames, setDiskNames] = useState<Record<string, string>>({});
  const [diskPools, setDiskPools] = useState<Record<string, string>>(() => Object.fromEntries(disks.map(disk => [disk.name, disk.pool])));
  ...
  const totalDiskBytes = disks.reduce((sum, disk) => sum + Math.max(0, disk.bytes || 0), 0) || vm.diskBytes;
  ...
  useEffect(() => {
    setInterfacePools(Object.fromEntries(interfaces.map(item => [item.name, networkPoolValueForInterface(item, networkPools)])));
  ...
  useEffect(() => {
    ...
    fetchNetworkPools(vm.hostId)
      .then(networkBody => {
        ...
        setInterfacePools(Object.fromEntries(interfaces.map(item => [item.name, networkPoolValueForInterface(item, networkBody.items)])));
      })
  ...
  async function submitClone() {
    ...
    if (currentCPU <= 0 || maximumCPU <= 0 || currentCPU > maximumCPU) return toast.warning('CPU 配置不正确');
    if (currentMemoryMB <= 0 || maximumMemoryMB <= 0 || currentMemoryMB > maximumMemoryMB) return toast.warning('内存配置不正确');
    if (hostCPU > 0 && maximumCPU > hostCPU) return toast.warning('最大 CPU 不能超过宿主机逻辑 CPU');
    if (hostMemoryMB > 0 && maximumMemoryMB > hostMemoryMB) return toast.warning('最大内存不能超过宿主机总内存');
    const payload: VMClonePayload = {
      ...
      interfaces: interfaces.map(item => ({
        ...
        source: networkPoolSourceForInterface(item, interfacePools[item.name] || item.source || '', networkPools),
      })),
    ...
    if (payload.interfaces.some(item => !item.mac)) return toast.warning('请输入所有网卡的新 MAC 地址');
    if (payload.interfaces.some(item => !item.source)) return toast.warning('请选择所有网卡的网络池');
    if (payload.disks.some(disk => !disk.pool)) return toast.warning('磁盘所属存储池未识别，无法克隆');
    if (payload.disks.some(disk => !disk.targetName)) return toast.warning('请输入克隆后的存储卷名称');
    ...
    <section className="grid h-full min-h-0 w-full gap-4 xl:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="min-h-0 space-y-3 overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Metric icon={MemoryStickIcon} label="内存" value={`${formatMemoryMB(sourceMemoryMB)} / ${formatMemoryMB(sourceMaxMemoryMB)}`} />
          <Metric icon={HardDriveIcon} label="磁盘" value={`${disks.length} 块 · ${formatBytes(totalDiskBytes, 'GB')}`} />
        ...
        <div className="flex h-full min-h-0 flex-col overflow-hidden">
          ...
          <Panel title="基础配置">
            <div className="grid gap-3 lg:grid-cols-2">
              <Field label={isTemplateMode ? '虚拟机名称' : '克隆名称'}>
                <input value={cloneName} disabled={busy} onChange={event => setCloneName(event.target.value)} placeholder={isTemplateMode ? 'new-vm-name' : vm.name} className={inputClass} style={fieldStyle} />
              </Field>
              <Field label="克隆后直接启动">
                <Toggle checked={autostart} disabled={busy} label={autostart ? '已启用' : '不启用'} onChange={setAutostart} />
              </Field>
              <Field label="描述" wide>
                <input value={description} disabled={busy} onChange={event => setDescription(event.target.value)} placeholder="可选" className={inputClass} style={fieldStyle} />
              </Field>
            </div>
          </Panel>
          ...
          <Panel title="资源配置">
            <div className="grid gap-3 lg:grid-cols-4">
              <NumberField label="当前 CPU" value={currentCPU} disabled={busy} onChange={setCurrentCPU} />
              <NumberField label="最大 CPU" value={maximumCPU} disabled={busy} onChange={setMaximumCPU} />
              <NumberField label="当前内存 MB" value={currentMemoryMB} disabled={busy} onChange={setCurrentMemoryMB} />
              <NumberField label="最大内存 MB" value={maximumMemoryMB} disabled={busy} onChange={setMaximumMemoryMB} />
            </div>
          ...
          <Panel title="网络配置">
            <div className="space-y-2">
              {interfaces.length ? (
                interfaces.map(item => (
                  <div key={item.name || item.mac} className="rounded-lg border p-3" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-control-bg-soft)' }}>
                    <div className="mx-auto grid w-full max-w-[820px] gap-3 md:grid-cols-[70px_130px_minmax(0,1fr)_150px] md:items-center">
                      <DeviceSummary className="text-center md:justify-self-center" title={interfaceName(item, vm)} tooltip={interfaceTooltip(item)} />
                      <SelectMenu
                        ...
                        onChange={value => setInterfacePools(current => ({ ...current, [item.name]: value }))}
                      />
                      <input value={macs[item.name] || ''} disabled={busy} onChange={event => setMacs(current => ({ ...current, [item.name]: event.target.value }))} className={inputClass + ' min-w-0'} style={fieldStyle} />
                      <button type="button" disabled={busy} onClick={() => setMacs(current => ({ ...current, [item.name]: randomMacAddress() }))} className="kvm-action-button flex h-9 min-w-0 w-full items-center justify-center gap-2 whitespace-nowrap rounded-lg border px-2.5 text-xs font-semibold disabled:opacity-60" style={{ background: 'rgba(59,130,246,0.12)', borderColor: 'rgba(59,130,246,0.38)', color: '#93c5fd' }}>
                        <ShuffleIcon size={15} />
                        随机 MAC 地址
                      </button>
                    </div>
          ...
          <Panel title="存储配置">
            <div className="space-y-2">
              {disks.map(disk => (
                <div key={disk.name} className="rounded-lg border p-3" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-control-bg-soft)' }}>
                  <div className="mx-auto grid w-full max-w-[820px] gap-3 md:grid-cols-[70px_130px_minmax(0,1fr)_105px] md:items-center">
                    <DeviceSummary className="text-center md:justify-self-center" title={diskName(disk)} tooltip={diskTooltip(disk)} />
                    <SelectMenu
                      ...
                      onChange={value => setDiskPools(current => ({ ...current, [disk.name]: value }))}
                    />
                    <input value={diskNames[disk.name] || ''} disabled={busy} onChange={event => setDiskNames(current => ({ ...current, [disk.name]: event.target.value }))} placeholder={diskPlaceholderName(vm.name, disk)} className={inputClass + ' min-w-0'} style={fieldStyle} />
                    {isQCOW2Disk(disk) ? (
                      <div className="flex h-9 w-full items-center justify-between gap-2 rounded-lg border px-3" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-control-bg)' }}>
                        <KvmTooltip label="预分配 qcow2 元数据" placement="top" align="center">
                          <span className="text-xs font-semibold" style={{ color: 'var(--kvm-text)' }} aria-label="Metadata 预分配 qcow2 元数据，写入更稳定">Metadata</span>
                        </KvmTooltip>
                        <MetadataCheckbox checked={Boolean(metadata[disk.name])} disabled={busy} onChange={checked => setMetadata(current => ({ ...current, [disk.name]: checked }))} />
                      </div>
                    ) : (
                      <span />
                    )}
          ...
          <Panel title="介质策略">
            <div className="grid gap-3 lg:grid-cols-[220px_1fr] lg:items-center">
              <SelectMenu
                ...
                onChange={value => setCDROMPolicy((autostart ? 'disconnect' : value) as CDROMPolicy)}
              />
              <div className="text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
                {autostart ? '克隆后直接启动时会自动断开介质' : `已连接介质 ${connectedCDROMs} 个，策略会应用到克隆后的 XML`}
              </div>
            </div>
          </Panel>
          </div>

        <div className="mt-3 flex shrink-0 justify-end border-t pt-3" style={{ borderColor: 'var(--kvm-border)' }}>
          <PrimaryButton label={busy ? (isTemplateMode ? '创建中' : '克隆中') : (isTemplateMode ? '创建' : '克隆')} disabled={busy} onClick={() => void submitClone()} />
        </div>
        </div>
      </div>
    </section>
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-xl border p-4" style={{ background: 'var(--kvm-control-bg-soft)', borderColor: 'var(--kvm-border)' }}>
      <h3 className="mb-3 text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>{title}</h3>
      {children}
    </section>
  );
}
...
function Field({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) {
  return (
    <label className={wide ? 'lg:col-span-2' : ''}>
      <div className="mb-1.5 text-xs font-semibold" style={{ color: 'var(--kvm-text-muted)' }}>{label}</div>
      {children}
    </label>
  );
}
...
function NumberField({ label, value, disabled, onChange }: { label: string; value: number; disabled?: boolean; onChange: (value: number) => void }) {
  return (
    <Field label={label}>
      <input value={value} type="number" min={1} disabled={disabled} onChange={event => onChange(Number(event.target.value))} className={inputClass} style={fieldStyle} />
    </Field>
  );
}
...
function Toggle({ checked, disabled, label, onChange }: { checked: boolean; disabled?: boolean; label: string; onChange: (value: boolean) => void }) {
  return (
    <button type="button" disabled={disabled} onClick={() => onChange(!checked)} className="kvm-action-button h-9 rounded-lg border px-3 text-sm font-semibold disabled:opacity-60" style={{ background: checked ? 'rgba(45,212,191,0.12)' : 'var(--kvm-control-bg)', borderColor: checked ? 'rgba(45,212,191,0.34)' : 'var(--kvm-border)', color: checked ? 'var(--kvm-check-toggle-active-text)' : 'var(--kvm-text-muted)' }}>
      {label}
    </button>
  );
}

function Metric({ icon: Icon, label, value }: { icon: typeof CpuIcon; label: string; value: string }) {
  return (
    <div className="rounded-xl border p-3" style={{ background: 'var(--kvm-control-bg-soft)', borderColor: 'var(--kvm-border)' }}>
      <div className="flex items-center gap-1.5 text-[11px]" style={{ color: 'var(--kvm-text-muted)' }}><Icon size={13} />{label}</div>
      <div className="mt-1 truncate text-xs font-semibold" style={{ color: 'var(--kvm-text)' }}>{value}</div>
    </div>
  );
}
...
function InfoLine({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3 text-xs">
      ...
      <span className="truncate font-mono" style={{ color: strong ? 'var(--kvm-text)' : 'var(--kvm-text-muted)' }}>{value}</span>
    </div>
  );
}

function DeviceSummary({ title, tooltip, className = '' }: { title: string; tooltip?: string; className?: string }) {
  const content = <div className="truncate text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>{title}</div>;
  return (
...
function MetadataCheckbox({ checked, disabled, onChange }: { checked: boolean; disabled?: boolean; onChange: (value: boolean) => void }) {
  return (
    <label className={'inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md ' + (disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer')}>
      <input type="checkbox" checked={checked} disabled={disabled} onChange={event => onChange(event.target.checked)} className="h-4 w-4 cursor-pointer accent-cyan-400 disabled:cursor-not-allowed" aria-label="Metadata" />
    </label>
  );
}
...
function networkPoolOptionsForInterface(iface: VMConfig['interfaces'][number], pools: NetworkPool[]) {
...
function networkPoolSourceForInterface(iface: VMConfig['interfaces'][number], value: string, pools: NetworkPool[]) {
...
function randomMacsByInterface(interfaces: VMConfig['interfaces'], current: Record<string, string> = {}) {
  return Object.fromEntries(interfaces.map(item => [item.name, current[item.name] || randomMacAddress()]));
}
...
function randomMacAddress() {
  const parts = Array.from({ length: 3 }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0'));
