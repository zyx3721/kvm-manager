...
export function MediaPanel({
  ...
  async function loadPools() {
    ...
    try {
      ...
      setPools(body.items);
      const current = selectedPool || body.items.find(pool => ['iso', 'dir'].includes((pool.type || '').toLowerCase()))?.name || '';
    ...
    <section className="mx-auto max-w-3xl space-y-4">
      <CardSection title="ISO 镜像选择">
        <div className="space-y-4">
          ...
          <div className="rounded-lg border p-3 text-sm" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-control-bg-soft)' }}>
            <div className="mb-2 flex items-center gap-2 font-semibold" style={{ color: 'var(--kvm-text)' }}>
              <Disc3Icon size={15} />当前介质
            </div>
            <div className="space-y-1.5">
              {displayCDROMs.map(cdrom => (
                <div key={cdrom.name || cdrom.path || 'cdrom'} className="grid gap-2 text-xs md:grid-cols-[90px_1fr]">
                  <span className="pl-10" style={{ color: 'var(--kvm-text)' }}>{cdrom.name || 'CDROM'}</span>
                  <span className="break-all font-mono" style={{ color: 'var(--kvm-text-muted)' }}>
                    {cdrom.connected ? `${cdrom.path || '-'}${cdrom.bus ? ` (${cdrom.bus})` : ''}` : '未连接介质'}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-[140px_1fr] md:items-center">
            <div className="text-right text-sm" style={{ color: 'var(--kvm-text)' }}>目标光驱</div>
            <SelectMenu
          ...
          <div className="grid gap-3 md:grid-cols-[140px_1fr] md:items-center">
            <div className="text-right text-sm" style={{ color: 'var(--kvm-text)' }}>存储池</div>
            <div className="flex gap-2">
              <SelectMenu
                value={selectedPool}
                disabled={controlsDisabled}
                placeholder="暂无 ISO 存储池"
                options={isoPools.map(pool => ({ value: pool.name, label: `${pool.name} · ${pool.path || pool.type}` }))}
                className="flex-1"
              ...
              <button
                ...
                style={buttonStyle}
              >
                <RefreshCwIcon size={15} />刷新
              </button>
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-[140px_1fr] md:items-center">
            <div className="text-right text-sm" style={{ color: 'var(--kvm-text)' }}>ISO 文件</div>
            <SelectMenu
              value={selectedISO}
              disabled={controlsDisabled || !selectedPool || isoFiles.length === 0}
              placeholder={loading ? '正在读取 ISO 文件' : '暂无 ISO 文件'}
              options={isoFiles.map(file => ({ value: file.path, label: `${file.name} · ${formatBytesAutoFixed(file.bytes)}` }))}
              maxVisibleItems={3}
              onChange={setSelectedISO}
            />
          </div>
          <div className="rounded-lg border p-3 text-sm" style={{ borderColor: 'var(--kvm-border)', background: 'var(--kvm-control-bg-soft)', color: 'var(--kvm-text-muted)' }}>
            <div className="mb-2 flex items-center gap-2 font-semibold" style={{ color: 'var(--kvm-text)' }}>
              <Disc3Icon size={15} />当前选择
            </div>
            <div className="pl-10 break-all font-mono text-xs">{selectedISO || '未选择 ISO 镜像'}</div>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            >
              <Disc3Icon size={15} />
              {mediaConnected ? (disconnecting ? '正在断开' : '断开') : connecting ? '正在连接' : '连接'}
            </button>
          </div>
        </div>
      </CardSection>
