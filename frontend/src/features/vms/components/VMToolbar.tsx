...
import { CheckIcon, ChevronDownIcon, Columns3Icon, DownloadIcon, FilterIcon, SearchIcon } from 'lucide-react';
    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          />
          <button type="button" onClick={onExport} className="kvm-action-button flex h-10 items-center gap-2 rounded-lg border px-3 text-sm" style={{ borderColor: 'rgba(59,130,246,0.38)', color: 'var(--kvm-accent-text)', background: 'rgba(59,130,246,0.1)' }}><DownloadIcon size={14} />导出</button>
...
function VMColumnPicker({
  ...
  const [menuRect, setMenuRect] = useState({ top: 0, left: 0, width: 0, placement: 'bottom' as 'bottom' | 'top' });
    ...
    <div ref={rootRef} className="relative">
      <button
        ...
        style={{
          ...
          background: 'var(--kvm-control-bg)',
          boxShadow: open ? '0 0 0 3px rgba(45,212,191,0.08)' : 'inset 0 1px 0 rgba(255,255,255,0.045)',
        }}
      ...
      {open && typeof document !== 'undefined' ? createPortal(
        ...
        >
          <div className="flex items-center justify-between gap-2 border-b px-1 pb-2" style={{ borderColor: 'var(--kvm-border)' }}>
            <div className="text-xs font-semibold" style={{ color: 'var(--kvm-text)' }}>当前视图列</div>
            <button
              type="button"
              onClick={toggleAll}
              className="kvm-action-button rounded-md border px-2 py-1 text-xs font-semibold"
              style={{ borderColor: 'var(--kvm-border)', color: 'var(--kvm-text-muted)', background: 'rgba(255,255,255,0.035)' }}
            >
              {allSelected ? '取消全选' : '全选'}
            </button>
          </div>
