...
export function VMMigrateDialog({
  ...
  const [sshDialog, setSSHDialog] = useState<{ payload: VMMigratePayload; message: string } | null>(
    null
  );
  const [hostnameDialog, setHostnameDialog] = useState<{
  ...
  const sourceCleanupDescription = copyDisks
    ? '迁移成功后删除源定义和源普通磁盘'
    : '避免源宿主机残留重复定义';
  ...
  const precheckPassed = Boolean(
    precheckReport?.passed && precheckPayloadKey === migrationPayloadKey
  );
  ...
  const migrationModeOptions = [
    {
      ...
      label: '热迁移',
      disabled: !running,
      tooltip: running ? '迁移运行中的虚拟机' : '运行中虚拟机可选择热迁移',
    },
    {
      value: 'cold',
      label: '冷迁移',
      disabled: running,
      tooltip: running ? '运行中虚拟机请使用热迁移' : '已停止虚拟机可选择冷迁移',
    },
  ];
        ...
        <div className="kvm-dialog-panel flex max-h-[88vh] w-[min(92vw,900px)] flex-col overflow-hidden rounded-2xl shadow-2xl">
          ...
          >
            ...
            <button
              ...
              className="kvm-action-button flex h-8 w-8 items-center justify-center rounded-lg border disabled:cursor-not-allowed disabled:opacity-60"
              style={{
          ...
          >
            <div className="kvm-dialog-card space-y-4 rounded-2xl p-4">
              <InlineNotice tone="warning">
                迁移前请确认目标宿主机具备兼容 CPU、同名网络桥接和可访问存储
              </InlineNotice>
              <Panel title="迁移对象">
                <div className="grid gap-3 md:grid-cols-5">
                  ...
                  <Metric
                    label="规格"
                    value={`${vm.cpuCores} vCPU / ${formatBytes(vm.memoryBytes, 'GB')}`}
                  />
                  <Metric
                    label="磁盘"
                    value={`${vm.disks.length || 1} 块 / ${formatBytes(vm.diskBytes, 'GB')}`}
                  />
                </div>
              </Panel>
              <Panel title="目标与方式">
              ...
              <Panel title="迁移策略">
                <div className="grid gap-3 sm:grid-cols-3">
                  <OptionToggle
                    label="复制本地磁盘"
                    description="本地磁盘迁移时启用，共享存储无需启用"
                    checked={copyDisks}
                    disabled={controlsLocked}
                    onChange={setCopyDisks}
                  />
                  <OptionToggle
                    label="持久化目标定义"
                    description="迁移后在目标宿主机保留定义"
                    checked={persistent}
                    disabled={controlsLocked}
                    onChange={setPersistent}
                  />
                  <OptionToggle
                    label={sourceCleanupLabel}
                    description={sourceCleanupDescription}
                    checked={undefineSource}
                    disabled={controlsLocked}
                    onChange={setUndefineSource}
                  />
                </div>
              </Panel>
              <Panel title="热迁移优化">
                <div className="grid gap-3 sm:grid-cols-2">
                  <OptionToggle
                    label="自动收敛"
                    description="降低迁移期间写入速度以便收敛"
                    checked={autoConverge}
                    disabled={controlsLocked || !live}
                    onChange={setAutoConverge}
                  />
                  <OptionToggle
                    label="Post-copy"
                    description="降低收敛等待，但失败恢复风险更高"
                    checked={postCopy}
                    disabled={controlsLocked || !live}
                    onChange={setPostCopy}
                  />
                </div>
                {!live && (
                  <div className="mt-3 text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
                    冷迁移不会使用自动收敛和 Post-copy 参数
                  </div>
                )}
              </Panel>
              {precheckReport && (
                <PrecheckPanel
                  ...
                  onSetupSSHKey={item =>
                    precheckPayload &&
                    setSSHDialog({ payload: precheckPayload, message: item.message })
                  }
                  onSetupHostname={item =>
                    precheckPayload &&
                    setHostnameDialog({ payload: precheckPayload, message: item.message })
                  }
                />
              )}
              <div className="flex flex-wrap justify-end gap-2">
                <SecondaryButton
                  label={prechecking ? '预检中' : '预检'}
                  disabled={controlsLocked}
                ...
                <KvmTooltip label={migrateDisabledReason} placement="top" align="end">
                  <span className="inline-flex">
                    <PrimaryButton
                      label={migrationSubmitting ? '提交中' : '迁移'}
                      disabled={controlsLocked || !precheckPassed}
                      onClick={() => void submit()}
                    />
                  </span>
                </KvmTooltip>
      ...
      <div className="grid gap-2 md:grid-cols-2">
        {report.items.map(item => (
          <div
            ...
            style={{ background: 'var(--kvm-control-bg)', borderColor: 'var(--kvm-border)' }}
          >
...
function canSetupMigrationSSHKey(item: VMMigrationPrecheckReport['items'][number]) {
  return (
    item.key === 'migration-channel' &&
    item.status === 'failed' &&
    item.code === 'vm_migrate_ssh_password_required'
  );
}
...
function canSetupMigrationHostname(item: VMMigrationPrecheckReport['items'][number]) {
  return (
    item.key === 'migration-channel' &&
    item.status === 'failed' &&
    item.code === 'vm_migrate_target_hostname_localhost'
  );
}
...
function PrecheckIcon({
  ...
  status: VMMigrationPrecheckReport['items'][number]['status'];
}) {
  if (status === 'passed')
    return (
      <CheckCircleIcon
        ...
        style={{ color: 'var(--kvm-status-green-text)' }}
      />
    );
  if (status === 'failed')
    return (
      <XCircleIcon
        size={15}
        className="mt-0.5 shrink-0"
        style={{ color: 'var(--kvm-status-red-text)' }}
      />
    );
  return (
    <CircleDashedIcon
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div
      className="kvm-card-hover rounded-lg border px-3 py-2"
      style={{ background: 'var(--kvm-control-bg)', borderColor: 'var(--kvm-border)' }}
    >
      <div className="text-xs font-semibold" style={{ color: 'var(--kvm-text-muted)' }}>
        {label}
      </div>
      <div className="mt-1 truncate text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>
        {value}
      </div>
    </div>
  );
...
function OptionToggle({
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-xs leading-5" style={{ color: 'var(--kvm-text-muted)' }}>
        {description}
      </div>
    </div>
  );
