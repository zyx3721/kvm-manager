...
type ChannelId = "webhook" | "email" | "lark" | "wechat" | "dingtalk" | "lark_app" | "wechat_app" | "dingtalk_app";
...
const channelMeta: Record<ChannelId, { name: string; description: string; icon: React.ElementType; color: string; fields: Field[] }> = {
  ...
  lark: {
    ...
    color: "#8b5cf6",
    fields: [
      { key: "webhookUrl", label: "机器人 Webhook", placeholder: "https://open.feishu.cn/open-apis/bot/v2/hook/...", required: true },
  ...
  lark_app: {
    ...
    fields: [
      ...
      { key: "appSecret", label: "App Secret", placeholder: "飞书应用密钥", required: true, type: "password" },
      { key: "receiveIdType", label: "接收 ID 类型", placeholder: "chat_id", required: true, helper: "支持 open_id、user_id、union_id、email、chat_id" },
  ...
  wechat: {
    ...
    color: "#22c55e",
    fields: [
      { key: "webhookUrl", label: "机器人 Webhook", placeholder: "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=...", required: true },
    ],
  },
  wechat_app: {
    name: "企业微信应用",
    ...
    fields: [
      ...
      { key: "secret", label: "应用 Secret", placeholder: "企业微信应用密钥", required: true, type: "password" },
      { key: "toUser", label: "接收成员", placeholder: "zhangsan|lisi", helper: "与部门、标签至少填写一项" },
  ...
  dingtalk: {
    ...
    color: "#f59e0b",
    fields: [
      { key: "webhookUrl", label: "机器人 Webhook", placeholder: "https://oapi.dingtalk.com/robot/send?access_token=...", required: true },
  ...
  dingtalk_app: {
    ...
    fields: [
      ...
      { key: "appSecret", label: "AppSecret", placeholder: "钉钉应用密钥", required: true, type: "password" },
      { key: "agentId", label: "AgentId", placeholder: "1000002", required: true },
      { key: "useridList", label: "用户列表", placeholder: "user001,user002", helper: "与部门列表至少填写一项" },
...
const larkCardColorOptions: SelectMenuOption[] = [
  { value: "red", label: <ColorOption color="#ef4444" label="红色" />, searchLabel: "红色", tooltip: "red" },
  { value: "green", label: <ColorOption color="#22c55e" label="绿色" />, searchLabel: "绿色", tooltip: "green" },
  { value: "blue", label: <ColorOption color="#3b82f6" label="蓝色" />, searchLabel: "蓝色", tooltip: "blue" },
  { value: "orange", label: <ColorOption color="#f97316" label="橙色" />, searchLabel: "橙色", tooltip: "orange" },
  { value: "yellow", label: <ColorOption color="#eab308" label="黄色" />, searchLabel: "黄色", tooltip: "yellow" },
  { value: "purple", label: <ColorOption color="#a855f7" label="紫色" />, searchLabel: "紫色", tooltip: "purple" },
  { value: "grey", label: <ColorOption color="#94a3b8" label="灰色" />, searchLabel: "灰色", tooltip: "grey" },
];
...
function NotificationConfigField({ field, value, form, disabled, onChange }: { field: Field; value: unknown; form: Record<string, unknown>; disabled: boolean; onChange: (value: unknown) => void }) {
  if (field.key === "receiveIdType") {
    return (
      <label className="block space-y-1.5 text-xs" style={{ color: "var(--kvm-text-muted)" }}>
        <span className="flex items-center gap-1">{field.label}<span style={{ color: "#f87171" }}>*</span></span>
        <SelectMenu
          ...
          onChange={onChange}
        />
        {field.helper && <span className="block text-[11px] leading-4" style={{ color: "var(--kvm-text-muted)" }}>{field.helper}</span>}
      </label>
    );
  }
  return <ConfigField field={field} value={value} secretConfigured={secretConfigured(field, form)} disabled={disabled} onChange={onChange} />;
}

function ColorOption({ color, label }: { color: string; label: string }) {
  return <span className="inline-flex min-w-0 items-center gap-2"><span className="h-3 w-3 shrink-0 rounded-full border" style={{ background: color, borderColor: "rgba(255,255,255,0.32)" }} /><span className="truncate">{label}</span></span>;
}
...
function TemplateEditor({ selected, tab, form, preview, busy, disabled, onPreview, onOpenVariables, onTabChange, onChange }: { selected: ChannelId; tab: TemplateTab; form: Record<string, unknown>; preview: NotificationTemplatePreview | null; busy: boolean; disabled: boolean; onPreview: () => void; onOpenVariables: () => void; onTabChange: (tab: TemplateTab) => void; onChange: (key: string, value: string) => void }) {
  ...
  const currentMessageType = messageType ? String(form[messageType.key] ?? messageType.options[0]?.value ?? "") : "";
  const isLarkChannel = selected === "lark" || selected === "lark_app";
  const showLarkTitle = isLarkChannel && (currentMessageType === "post" || currentMessageType === "interactive");
    ...
    <section className="mt-5 rounded-lg border p-3" style={{ borderColor: "var(--kvm-border)", background: "rgba(255,255,255,0.026)" }}>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative grid h-9 w-[128px] grid-cols-2 overflow-hidden rounded-lg border p-1" style={{ borderColor: "var(--kvm-border)", background: "rgba(255,255,255,0.03)" }}>
            <span className="pointer-events-none absolute bottom-1 top-1 w-[calc(50%-4px)] rounded-md transition-transform duration-1000 ease-in-out" style={{ left: 4, transform: tab === "recovery" ? "translateX(100%)" : "translateX(0)", background: "rgba(59,130,246,0.14)" }} />
      ...
      {showLarkTitle && <TemplateInlineInput label="文本标题" value={String(form[larkTitleKey] ?? "")} placeholder={defaultSubject} disabled={disabled} onChange={(value) => onChange(larkTitleKey, value)} />}
...
function PreviewBlock({ title, value }: { title: string; value: string }) {
  return <div className="mt-2"><div className="mb-1 text-[11px]" style={{ color: "var(--kvm-text-muted)" }}>{title}</div><pre className="kvm-hidden-scrollbar max-h-44 overflow-auto whitespace-pre-wrap rounded-lg border p-2 font-mono text-xs leading-5" style={{ borderColor: "var(--kvm-border)", color: "var(--kvm-text)", background: "var(--kvm-control-bg)" }}>{value || "-"}</pre></div>;
}

function TemplateFieldLabel({ label, className = "", children }: { label: string; className?: string; children: React.ReactNode }) {
  return <label className={`block max-w-full text-xs ${className}`} style={{ color: "var(--kvm-text-muted)" }}><span className="mb-1.5 block">{label}</span>{children}</label>;
}

function TemplateInlineInput({ label, value, placeholder, disabled, onChange }: { label: string; value: string; placeholder: string; disabled: boolean; onChange: (value: string) => void }) {
  return <TemplateFieldLabel label={label}><input value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="h-10 w-full rounded-lg px-3 text-sm outline-none disabled:opacity-60" style={{ background: "var(--kvm-control-bg)", border: "1px solid var(--kvm-border)", color: "var(--kvm-text)" }} /></TemplateFieldLabel>;
}
          ...
          <div className="kvm-hidden-scrollbar min-h-0 space-y-4 overflow-y-auto pr-1">
            {templateVariableGroups.map((group) => (
              <section key={group.title}>
                <div className="mb-2 flex items-center gap-2 text-xs font-semibold" style={{ color: "var(--kvm-text)" }}>
                  <span>{group.title}</span>
                  <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full border px-1.5 text-[10px]" style={{ borderColor: "var(--kvm-border)", color: "var(--kvm-text-muted)", background: "rgba(148,163,184,0.08)" }}>{group.variables.length}</span>
                </div>
...
function TemplateTextarea({ label, value, placeholder, disabled, rows = 4, onChange }: { label: string; value: string; placeholder: string; disabled: boolean; rows?: number; onChange: (value: string) => void }) {
  return <TemplateFieldLabel label={label} className="mt-3"><textarea value={value} disabled={disabled} rows={rows} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="kvm-hidden-scrollbar w-full resize-y rounded-lg px-3 py-2 font-mono text-xs leading-5 outline-none disabled:opacity-60" style={{ background: "var(--kvm-control-bg)", border: "1px solid var(--kvm-border)", color: "var(--kvm-text)" }} /></TemplateFieldLabel>;
}

function TemplateTabButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="relative z-[1] h-7 cursor-pointer rounded-md px-3 text-xs transition-colors duration-300" style={{ color: active ? "var(--kvm-accent-hover)" : "var(--kvm-text-muted)" }}>{label}</button>;
}
...
function prepareConfig(id: ChannelId, form: Record<string, unknown>, enabled: boolean): { config: Record<string, unknown>; error: string } {
  ...
  if (id === "lark_app") {
    ...
    if (missingField) return { config: {}, error: `${missingField.label}不能为空` };
    if (!["open_id", "user_id", "union_id", "email", "chat_id"].includes(String(next.receiveIdType ?? ""))) {
  ...
  if (id === "wechat_app") {
    ...
    if (missingField) return { config: {}, error: `${missingField.label}不能为空` };
    if (!String(next.toUser ?? "").trim() && !String(next.toParty ?? "").trim() && !String(next.toTag ?? "").trim()) {
