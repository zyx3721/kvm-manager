...
export function SettingsSplitLayout({
  ...
  return (
    <div
      className="grid min-h-0 flex-1 grid-cols-1 items-stretch gap-4 overflow-hidden xl:grid-cols-[var(--settings-nav-column)_minmax(0,1fr)]"
      style={{ '--settings-nav-column': settingsNavColumnWidth } as React.CSSProperties}
    >
      <nav
    ...
    <div className="mb-4 flex items-center gap-3">
      ...
      <div>
        ...
        <div
          ...
          style={{ color: active ? '#86efac' : 'var(--kvm-text-muted)' }}
        >
          {subtitle}
        </div>
      </div>
...
export function EnableMediaToggle({
  enabled,
  disabled,
  onChange,
  label = '启用媒介',
  enabledText = '已开启外部通知推送',
  disabledText = '关闭后不会发送外部通知',
}: {
  enabled: boolean;
  disabled: boolean;
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        ...
        <span className="mt-0.5 block text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
      ...
      {field.type === 'textarea' ? (
        ...
        <span className="relative block">
          <input
            ...
            className={`w-full rounded-lg px-3 py-2 text-sm outline-none disabled:opacity-60 ${field.type === 'password' ? 'pr-10' : ''} ${field.type === 'number' ? 'kvm-number-input' : ''}`}
            style={commonStyle}
          />
          {field.type === 'password' && (
            <KvmTooltip
...
export function normalizeConfig(
  config: NotificationChannel['config'] | AuthProvider['config'] | undefined
) {
  if (!config) return {};
...
export function displayValue(field: Field, value: unknown) {
  if (field.key === 'headers' && value && typeof value !== 'string')
