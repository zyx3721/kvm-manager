  ...
  const saveUser = async () => {
    ...
    try {
      ...
      const saved = userForm.id
        ? await updateManagedUser(userForm.id, payload)
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
          ...
          {canCreate && (
            <button
              type="button"
              onClick={onCreate}
              className="kvm-action-button flex h-9 shrink-0 items-center gap-2 rounded-lg border px-3 text-sm"
              style={{
...
function ListPane({
  ...
  return (
    <div className="kvm-hidden-scrollbar min-h-0 flex-1 overflow-y-auto p-3">
      {total === 0 ? (
...
function UserEditorDialog({
  ...
  onSave: () => void;
  onDelete: () => void;
  onToggleDisabled: (user: ApiUser) => void;
}) {
        ...
        canManage ? (
          <>
            {form.id && (
              <DangerButton
                busy={busy === 'user'}
                disabled={!form.disabled || busy !== '' || defaultAdmin}
                onClick={onDelete}
                label="删除"
              />
            )}
  ...
  return (
    <DialogFrame
      ...
      footer={
        canManage ? (
          <>
            {form.id && <DangerButton busy={busy === 'group'} onClick={onDelete} label="删除" />}
            <StateButton
...
function DialogFrame({
  ...
  const node = (
    <div
      className="kvm-dialog-backdrop fixed inset-0 z-[1500] flex items-center justify-center px-3 py-5"
      role="presentation"
    >
      <div
        className="kvm-dialog-panel flex max-h-[88vh] w-[min(92vw,720px)] flex-col overflow-hidden rounded-2xl shadow-2xl"
        role="dialog"
        aria-modal="true"
        aria-label={title}
      >
        ...
        >
          <div className="min-w-0">
            <h3 className="truncate text-base font-semibold" style={{ color: 'var(--kvm-text)' }}>
              {title}
            </h3>
            <p className="mt-1 truncate text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
              {subtitle}
            </p>
          </div>
          <button
        ...
        <div className="kvm-hidden-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto p-5">
          {children}
        </div>
        {footer && (
          <div
...
function CompactRow({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="kvm-action-button group grid min-h-[74px] w-full grid-cols-[3px_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border p-0 pr-3 text-left"
      style={{
        background: active ? 'rgba(59,130,246,0.11)' : 'rgba(255,255,255,0.026)',
        borderColor: active ? 'rgba(96,165,250,0.52)' : 'var(--kvm-border)',
        color: 'var(--kvm-text)',
      }}
    >
      <span
      ...
      <span className="min-w-0 py-2">
        ...
        <span className="mt-1 block truncate text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
          {meta}
        </span>
        <span className="mt-0.5 block truncate text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
          {extra}
        </span>
      </span>
      {typeof enabled === 'boolean' ? (
        enabled ? (
          <ToggleRightIcon size={18} style={{ color: '#86efac' }} />
        ) : (
          <ToggleLeftIcon size={18} style={{ color: 'var(--kvm-text-muted)' }} />
        )
      ) : (
        <span
          className="h-2 w-2 rounded-full"
          style={{ background: active ? '#60a5fa' : 'var(--kvm-border)' }}
        />
      )}
    </button>
  );
...
function SectionTitle({ title }: { title: string }) {
  return (
    <div className="pt-1 text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>
      {title}
    </div>
  );
}
        ...
        {isPassword && (
          ...
          >
            <button
              ...
              style={{ color: 'var(--kvm-text-muted)', background: 'rgba(255,255,255,0.035)' }}
...
function CheckList({
  ...
  const listMaxHeight = maxVisibleItems
    ? maxVisibleItems * 74 + Math.max(0, maxVisibleItems - 1) * 6
    : undefined;
  return (
    <div className="space-y-2">
        ...
        {!hideActions && (
          <div className="flex gap-1">
            <button
              ...
              onClick={selectAll}
              className="kvm-action-button cursor-pointer rounded-md border px-2.5 py-1 text-[11px] transition disabled:cursor-not-allowed disabled:opacity-50"
              style={{
      ...
      <div
        className={`${maxVisibleItems ? '' : compact ? 'max-h-48' : 'max-h-52'} kvm-hidden-scrollbar grid gap-1.5 overflow-y-auto pr-1`}
      ...
      >
        {items.map(item => {
          ...
          return (
            <label
              key={item.key}
              className={`flex items-start gap-2 rounded-lg border ${compact ? 'p-2' : 'p-2'} text-sm ${disabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}
              style={{
                background: checked ? 'rgba(59,130,246,0.1)' : 'rgba(255,255,255,0.026)',
                borderColor: checked ? 'rgba(96,165,250,0.45)' : 'var(--kvm-border)',
                color: 'var(--kvm-text)',
              }}
            >
              <input
              ...
              {checked && (
                <CheckIcon className="ml-auto shrink-0" size={15} style={{ color: '#93c5fd' }} />
              )}
            </label>
          );
        })}
