      ...
      >
        <div className="kvm-dialog-panel w-full max-w-md rounded-xl p-5">
          ...
          <div className="mx-auto mt-5 max-w-sm space-y-4">
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder={file?.name || 'ubuntu-22.04.iso'}
          ...
          <DialogFooter
            busy={busy}
            confirmLabel="上传"
            disabled={!file || (displayName !== '' && !displayName.toLowerCase().endsWith('.iso'))}
            onClose={onClose}
            onConfirm={() => file && onSubmit(file, displayName)}
          />
        </div>
      </div>
    </DialogPortal>
