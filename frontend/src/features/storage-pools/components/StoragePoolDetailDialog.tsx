...
export function StoragePoolDetailDialog({
  agentId,
  hostName,
  item,
  canManage,
  ...
  const usage =
    localItem.capacity > 0 ? Math.round((localItem.allocation * 100) / localItem.capacity) : 0;
  ...
  const filteredVolumes = useMemo(() => {
    ...
    return volumes.filter(volume =>
      [
        ...
        volume.type,
        volume.format,
        formatBytesAutoFixed(volume.capacity),
      ].some(value =>
        String(value || '')
          .toLowerCase()
          .includes(query)
  ...
  async function uploadISO(file: File, name: string) {
    ...
    setBusy('upload');
    try {
      const completed = await runStorageISOUpload({
        agentId,
        poolName: localItem.name,
        file,
        targetName,
      });
      if (completed) {
          ...
          >
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
                {hostName} · {localItem.type || 'unknown'}
              </p>
            </div>
            <button
              type="button"
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    background: storageUsageColor(usage),
                  }}
                />
              </div>
            </section>
            ...
            <section className="mt-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  {isIsoPool(localItem) ? (
                    <Disc3Icon size={18} style={{ color: '#60a5fa' }} />
                  ) : (
                    <HardDriveIcon size={18} style={{ color: '#2dd4bf' }} />
                  )}
                  <h3 className="text-lg font-semibold" style={{ color: 'var(--kvm-text)' }}>
                    {volumeKind}
                  </h3>
                </div>
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <div className="relative w-[min(260px,calc(94vw-40px))]">
                    ...
                    <input
                      value={volumeQuery}
                      onChange={event => setVolumeQuery(event.target.value)}
                      placeholder={`搜索${volumeKind}`}
                      className="h-9 w-full rounded-lg pl-9 pr-9 text-sm outline-none"
                      style={{
                    ...
                    {volumeQuery && (
                      <button
                        type="button"
                        onClick={() => setVolumeQuery('')}
                        aria-label="清空搜索"
                        className="kvm-action-button absolute right-2 top-1/2 inline-flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded-md"
                        style={{ color: 'var(--kvm-text-muted)' }}
                      >
                        <XIcon size={13} />
                      </button>
...
function ActionTile({
  ...
  return (
    <div
      className="rounded-lg border p-3"
      style={{
        borderColor: actionColor(buttonTone).border,
        background: actionColor(buttonTone).background,
      }}
    >
      <div className="text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
        {label}
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            style={{
              borderColor: actionColor(buttonTone).border,
              background: actionColor(buttonTone).buttonBg,
              color: actionColor(buttonTone).text,
            }}
          >
            {buttonLabel}
          </button>
...
function DeletePoolButton({
  ...
  return (
    <KvmTooltip label="运行中的存储池需要先停止后才能删除" placement="top">
      {button}
    </KvmTooltip>
  );
}
...
function VolumeTable({
  ...
  return (
    ...
    >
      <div
        className={
          'kvm-hidden-scrollbar overflow-x-auto ' + (volumes.length > 5 ? 'overflow-y-auto' : '')
        }
        style={volumes.length > 5 ? { maxHeight: '342px' } : undefined}
      >
        <table className="w-full min-w-[720px] border-collapse text-sm">
          ...
          <tbody style={{ color: 'var(--kvm-text-muted)' }}>
            {loading && (
              <tr>
                <td colSpan={5} className="px-3 py-8 text-center">
                  <LoaderCircleIcon size={18} className="mx-auto animate-spin" />
                </td>
              </tr>
            )}
            {!loading && volumes.length === 0 && (
              <tr>
                <td colSpan={5} className="px-3 py-8 text-center">
                  {emptyText || (inactive ? '存储池已停止，暂无卷列表' : '暂无任何卷')}
                </td>
              </tr>
            )}
            {!loading &&
              volumes.map((volume, index) => (
                <tr key={volume.name} style={{ borderTop: '1px solid var(--kvm-border)' }}>
                  ...
                  <td
                    className="max-w-[420px] truncate px-3 py-3 text-center font-semibold"
                    style={{ color: 'var(--kvm-text)' }}
                  >
                    {volume.name}
                  </td>
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex justify-center gap-2">
                      {canManage && !iso && (
                        <KvmTooltip
                          label={volume.cloneSupported ? '克隆当前镜像' : '该格式暂不支持克隆'}
                          placement="top"
                        >
                          <button
                            type="button"
                            disabled={!volume.cloneSupported || cloneName === volume.name}
                            onClick={() => onClone(volume)}
                            className="kvm-action-button inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs disabled:opacity-50"
                            style={{
                              borderColor: 'rgba(59,130,246,0.34)',
                              color: 'var(--kvm-status-blue-text)',
                      ...
                      {canManage && (
                        <button
                          type="button"
                          disabled={!volume.deleteSupported || deleteName === volume.name}
                          onClick={() => void onDelete(volume)}
                          className="kvm-action-button kvm-danger-button inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs disabled:opacity-50"
                          style={{
                            borderColor: 'rgba(239,68,68,0.35)',
                            color: '#f87171',
                            background: 'rgba(239,68,68,0.1)',
          ...
          <div className="mx-auto mt-5 max-w-sm space-y-4">
            ...
            <FormField label="格式">
              <SelectMenu
                value={format}
                placeholder="请选择格式"
                options={['qcow2', 'qcow', 'qed', 'raw'].map(item => ({
                  value: item,
                  label: item,
                }))}
                onChange={setFormat}
              ...
              >
                <span>
                  ...
                  <span className="mt-1 block text-xs" style={{ color: 'var(--kvm-text-muted)' }}>
                    创建 qcow2 镜像时启用 metadata 预分配
                  </span>
                </span>
                <input
                  type="checkbox"
                  checked={preallocMetadata}
                  onChange={event => setPreallocMetadata(event.target.checked)}
            ...
            <FormField label="容量">
              <div className="grid grid-cols-[1fr_104px] gap-2">
                ...
                <SelectMenu
                  value={unit}
                  placeholder="单位"
                  options={['MB', 'GB', 'TB'].map(item => ({ value: item, label: item }))}
                  onChange={setUnit}
                />
              </div>
            </FormField>
            ...
            >
              {Number(capacity) > 0
                ? `将创建 ${volumeName}，容量 ${formatBytesAuto(bytes)}${metadataEnabled ? '，并预分配元数据' : ''}`
                : '请输入容量'}
            </div>
          </div>
          <DialogFooter
            busy={busy}
            confirmLabel="创建"
            disabled={!trimVolumeExtension(name.trim()) || bytes <= 0}
            onClose={onClose}
            onConfirm={() =>
              onSubmit({
                name: volumeName,
                format,
                capacityBytes: bytes,
                preallocMetadata: metadataEnabled,
              })
            }
          />
        </div>
...
function trimVolumeExtension(name: string) {
  return name.replace(/\.(qcow2|qcow|qed|raw|img)$/i, '');
}

function volumeExtension(format: string) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={primaryButtonStyle}
      >
        {busy && <LoaderCircleIcon size={14} className="animate-spin" />}
        {confirmLabel}
      </button>
    </div>
  );
...
function FormField({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-semibold" style={{ color: 'var(--kvm-text)' }}>
        {label}
      </span>
      {children}
    </label>
...
function isIsoPool(item: StoragePool) {
  return (
    item.type.toLowerCase() === 'iso' ||
    item.name.toLowerCase().includes('iso') ||
    item.path.toLowerCase().includes('iso')
  );
}
