...
> = {
  tasks: {
    title: '任务载荷筛选',
    keyPlaceholder: '字段，如 vm',
    valuePlaceholder: '值，如 test',
    sourceName: '任务载荷',
  },
  audit: {
    title: '审计元数据筛选',
    keyPlaceholder: '字段，如 agent',
    valuePlaceholder: '值，如 test',
    sourceName: '审计元数据',
  },
  alerts: {
    title: '告警元数据筛选',
    keyPlaceholder: '字段，如 metric',
    valuePlaceholder: '值，如 disk',
    sourceName: '告警元数据',
  },
};
...
export default function OperationsPage() {
  ...
  const start =
    effectiveTotal === 0 ? 0 : pageSize === 'all' ? 1 : (currentPage - 1) * pageSize + 1;
  const end =
    pageSize === 'all'
      ? effectiveTotal
      : Math.min(effectiveTotal, (currentPage - 1) * pageSize + visibleCount);
  ...
  return (
    <div data-cmp="OperationsPage" className="space-y-5 p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div
          className="flex w-fit flex-wrap gap-2 rounded-xl p-2"
          ...
          {tabs.map(item => {
            ...
            return (
              <button
                ...
                className="kvm-action-button flex items-center gap-2 rounded-lg px-3 py-2 text-sm"
                style={{
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="kvm-action-button flex h-10 shrink-0 items-center gap-2 rounded-lg border px-3 text-sm"
            style={{
              background: hasAdvancedFilter ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.03)',
              borderColor:
                hasAdvancedFilter || advancedOpen ? 'rgba(59,130,246,0.38)' : 'var(--kvm-border)',
              color:
          ...
          >
            ...
            {hasAdvancedFilter && (
              <span
                ...
                style={{ background: 'rgba(59,130,246,0.16)', color: 'var(--kvm-accent-text)' }}
              >
                已启用
              </span>
            )}
          ...
          {tab === 'alerts' && (
            <Segmented value={status} onChange={setStatus} items={alertStatuses} />
          )}
        </div>
        {advancedOpen && (
          <AdvancedJsonFilter
            filter={currentJsonFilter}
      ...
      {!loading && tab === 'audit' && (
        <AuditTable items={visibleLogs} onDetail={item => setDetail({ type: 'audit', item })} />
      )}
      {!loading && tab === 'alerts' && (
        <AlertTable
          items={visibleAlerts}
          ...
          onDetail={item => setDetail({ type: 'alerts', item })}
        />
      )}
      {!loading && (
        <Pagination
...
function TaskTable({ items, onDetail }: { items: Task[]; onDetail: (item: Task) => void }) {
  return (
    <RecordShell fixed>
      <colgroup>
        <col className="w-[15%]" />
      ...
      <tbody>
        {items.length === 0 && <Empty colSpan={6} text="暂无任务记录" />}
        {items.map(item => (
          <tr key={item.id} style={{ borderBottom: '1px solid rgba(56,78,120,0.16)' }}>
            ...
            <Cell>{formatTimeAgo(item.created_at)}</Cell>
            <Cell>
              <TooltipButton label="查看详情" onClick={() => onDetail(item)}>
                <EyeIcon size={14} />
              </TooltipButton>
            </Cell>
          </tr>
...
function AuditTable({
  ...
  return (
    <RecordShell>
      ...
      <tbody>
        {items.length === 0 && <Empty colSpan={6} text="暂无审计日志" />}
        {items.map(item => (
          <tr key={item.id} style={{ borderBottom: '1px solid rgba(56,78,120,0.16)' }}>
...
function AlertTable({
  ...
  busyId: string;
  canManage: boolean;
  onResolve: (item: Alert) => void;
  onDetail: (item: Alert) => void;
}) {
  return (
    <RecordShell>
      <thead>
        <tr>
          {['级别', '状态', '标题', '来源', '外部通知', '时间', '操作'].map(head => (
            <Head key={head}>{head}</Head>
          ))}
        </tr>
      </thead>
      <tbody>
        {items.length === 0 && <Empty colSpan={7} text="暂无告警" />}
        {items.map(item => (
          <tr key={item.id} style={{ borderBottom: '1px solid rgba(56,78,120,0.16)' }}>
            <Cell main>
              <AlertLevelText level={item.level} />
            </Cell>
            <Cell>
              <StatusPill value={labelStatus(item.status)} tone={statusTone(item.status)} />
            </Cell>
            ...
            <Cell>
              {labelResource(item.sourceType)}/{item.sourceId}
            </Cell>
            <Cell>{item.notificationSentAt ? '已触达' : '待发送'}</Cell>
            <Cell>{formatTimeAgo(item.lastSeenAt)}</Cell>
            <Cell>
...
function RecordShell({ children, fixed }: { children: React.ReactNode; fixed?: boolean }) {
  return (
    <div
    ...
    >
      <div className="overflow-x-auto">
        <table className={`${fixed ? 'table-fixed' : ''} w-full text-sm`}>{children}</table>
      </div>
    </div>
  );
}
function Head({ children }: { children: React.ReactNode }) {
  return (
    <th
      ...
      style={{ color: 'var(--kvm-text-muted)', borderBottom: '1px solid var(--kvm-border)' }}
    >
      {children}
    </th>
  );
...
function Cell({
  ...
  return (
    <td
      className={`${compact ? 'px-2' : 'px-4'} py-3 text-center`}
      style={{ color: main ? 'var(--kvm-text)' : 'var(--kvm-text-muted)' }}
    >
      {children}
    </td>
  );
...
function AdvancedJsonFilter({
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(255,255,255,0.025)', borderColor: 'var(--kvm-border)' }}
    >
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        {active && (
          <button
            type="button"
            onClick={onClear}
            className="kvm-action-button rounded-md border px-2 py-1 text-xs"
            style={{
      ...
      <div className="mt-2 text-[11px]" style={{ color: 'var(--kvm-text-muted)' }}>
        {filter.key.trim()
  ...
  return (
    <>
      <button
        ...
        onClick={onClick}
        className="kvm-action-button inline-flex h-8 w-8 items-center justify-center rounded-lg border disabled:opacity-40"
        style={{
          borderColor: success ? 'rgba(16,185,129,0.35)' : 'var(--kvm-border)',
          color: success ? '#34d399' : 'var(--kvm-text-muted)',
      ...
      {tooltip.open && (
        <div
          className="pointer-events-none fixed z-[80] -translate-x-1/2 text-left shadow-2xl"
          style={{
            left: tooltip.left,
            top: tooltip.top,
            transform:
    ...
    <div className="flex flex-wrap gap-2">
      {items.map(item => (
        <button
          ...
          className="kvm-action-button rounded-lg px-3 py-2 text-xs"
          style={{
            background: value === item.value ? 'rgba(59,130,246,0.15)' : 'transparent',
            border:
              value === item.value
                ? '1px solid rgba(59,130,246,0.32)'
                : '1px solid var(--kvm-border)',
            color: value === item.value ? 'var(--kvm-accent-text)' : 'var(--kvm-text-muted)',
          }}
        >
          {item.label}
        </button>
...
function PageSizePicker({
  ...
  const current = pageSizes.find(item => item.value === value) ?? pageSizes[1];
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} className="relative flex items-center gap-2">
      ...
      <button
        ...
        className="kvm-action-button flex h-10 w-28 cursor-pointer items-center justify-between rounded-lg border px-3 text-left"
        style={{
          background: 'rgba(255,255,255,0.045)',
          borderColor: open ? 'rgba(96,165,250,0.48)' : 'var(--kvm-border)',
          color: 'var(--kvm-text)',
        }}
      >
      ...
      {open && (
        <div
          className="absolute right-0 top-[calc(100%+8px)] z-40 w-28 overflow-hidden rounded-lg border p-1 shadow-2xl"
          style={{
        ...
        >
          {pageSizes.map(item => (
            <button
              ...
              className="flex h-10 w-full cursor-pointer items-center justify-between rounded-md px-3 text-left text-sm font-semibold"
              style={{
                background: item.value === value ? 'rgba(96,165,250,0.16)' : undefined,
                color: item.value === value ? '#bfdbfe' : 'var(--kvm-text)',
              }}
            >
              <span>{item.label}</span>
              {item.value === value && <CheckIcon size={15} />}
            </button>
          ))}
        </div>
      )}
...
function PageButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="kvm-action-button flex h-9 w-9 items-center justify-center rounded-lg border disabled:opacity-40"
      style={{
...
function DetailDialog({ detail, onClose }: { detail: DetailItem; onClose: () => void }) {
  ...
  const title =
    detail.type === 'tasks' ? '任务详情' : detail.type === 'audit' ? '审计详情' : '告警详情';
  ...
  return (
    <DialogPortal>
      <div className="kvm-dialog-backdrop fixed inset-0 z-50 flex items-center justify-center px-4">
        <div className="kvm-dialog-panel kvm-operations-dialog max-h-[86vh] w-full max-w-4xl overflow-hidden rounded-xl p-5 shadow-2xl">
          ...
          <div className="kvm-hidden-scrollbar max-h-[74vh] overflow-y-auto pr-1">
            ...
            {extra && (
              <div className="mt-4">
                ...
                <pre
                  className="kvm-payload-pre kvm-hidden-scrollbar max-h-[32vh] overflow-y-auto overflow-x-hidden whitespace-pre-wrap break-all rounded-lg p-4 text-xs"
                  style={{ color: 'var(--kvm-text-muted)' }}
                >
                  {extra.value}
                </pre>
  ...
  return (
    <section className="mt-4">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div className="text-xs font-semibold" style={{ color: 'var(--kvm-text-muted)' }}>
          通知投递历史
        </div>
        {loading && (
      ...
      >
        <table className="w-full text-xs">
          ...
          <tbody>
            {!loading && error && (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center" style={{ color: '#fca5a5' }}>
                  {error}
                </td>
              </tr>
            )}
            ...
            {items.map(item => (
              <tr key={item.id} style={{ borderBottom: '1px solid rgba(56,78,120,0.16)' }}>
                <td className="px-3 py-2 text-center" style={{ color: 'var(--kvm-text)' }}>
                  {labelChannel(item.channelId)}
                </td>
                <td className="px-3 py-2 text-center" style={{ color: 'var(--kvm-text-muted)' }}>
                  {item.eventType === 'recovery' ? '恢复' : '告警'}
                </td>
                <td className="px-3 py-2 text-center">
                  <StatusPill
                    value={labelDeliveryStatus(item.status)}
                ...
                <td className="px-3 py-2 text-center" style={{ color: 'var(--kvm-text-muted)' }}>
                  ...
                  {item.status === 'pending' && item.retryCount > 0
                    ? ` / ${fullTime(item.nextRetryAt)}`
                    : ''}
                </td>
                <td className="px-3 py-2 text-center" style={{ color: 'var(--kvm-text-muted)' }}>
                  {item.sentAt
                    ? fullTime(item.sentAt)
                    : item.lastAttemptAt
                      ? fullTime(item.lastAttemptAt)
                      : fullTime(item.created_at)}
                </td>
                <td
                  className="max-w-[280px] px-3 py-2 text-center"
                  style={{ color: item.error ? '#fca5a5' : 'var(--kvm-text-muted)' }}
                >
                  <span className="line-clamp-2 break-words">{item.error || '-'}</span>
                </td>
              </tr>
            ))}
          </tbody>
...
function ReadOnlyField({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="kvm-dialog-card kvm-operations-detail-card rounded-lg p-3">
      <div className="mb-1 text-[11px]" style={{ color: 'var(--kvm-text-muted)' }}>
        {label}
      </div>
      <div className="break-words text-sm font-medium" style={{ color: 'var(--kvm-text)' }}>
        {value || '-'}
      </div>
    </div>
  );
...
function StatusPill({
  ...
  const colors = {
    green: ['var(--kvm-status-green-text)', 'rgba(16,185,129,0.12)', 'rgba(16,185,129,0.24)'],
    red: ['var(--kvm-status-red-text)', 'rgba(239,68,68,0.12)', 'rgba(239,68,68,0.24)'],
    yellow: ['var(--kvm-status-yellow-text)', 'rgba(245,158,11,0.12)', 'rgba(245,158,11,0.24)'],
    blue: ['var(--kvm-status-blue-text)', 'rgba(59,130,246,0.12)', 'rgba(59,130,246,0.24)'],
    gray: ['var(--kvm-status-gray-text)', 'rgba(148,163,184,0.1)', 'rgba(148,163,184,0.18)'],
  }[tone];
  return (
    <span
...
function AlertLevelText({ level }: { level: string }) {
  return (
    <span
      className="inline-flex items-center justify-center gap-1.5 font-semibold"
      style={{ color: alertLevelColor(level) }}
    >
      <AlertTriangleIcon size={14} />
      {labelAlertLevel(level)}
    </span>
  );
}
...
function detailRows(detail: NonNullable<DetailItem>) {
  if (detail.type === 'tasks') {
    const item = detail.item as Task;
    return [
      { label: '任务 ID', value: item.id },
      { label: '任务类型', value: labelTaskType(item.type) },
      { label: '状态', value: labelStatus(item.status) },
      { label: '目标', value: `${item.targetType || '-'}/${item.targetId || '-'}` },
      { label: '进度', value: taskProgress(item) },
      { label: '错误信息', value: item.errorMessage || '-' },
      { label: '创建时间', value: fullTime(item.created_at) },
      { label: '完成时间', value: item.finished_at ? fullTime(item.finished_at) : '-' },
    ];
  }
  if (detail.type === 'audit') {
    const item = detail.item as AuditLog;
    return [
      { label: '审计 ID', value: item.id },
      { label: '动作', value: labelAction(item.action) },
      { label: '用户', value: item.username || '-' },
      { label: '用户 ID', value: item.userId || '-' },
      { label: '资源', value: `${item.resourceType}/${item.resourceId || '-'}` },
      { label: 'IP 地址', value: item.ipAddress || '-' },
      { label: '发生时间', value: fullTime(item.created_at) },
    ];
  }
  const item = detail.item as Alert;
  return [
    { label: '告警 ID', value: item.id },
    { label: '级别', value: <AlertLevelText level={item.level} /> },
    { label: '状态', value: labelStatus(item.status) },
    { label: '标题', value: item.title },
    { label: '消息', value: item.message },
    { label: '来源', value: `${labelResource(item.sourceType)}/${item.sourceId}` },
    {
      label: '外部通知',
      value: item.notificationSentAt ? fullTime(item.notificationSentAt) : '待发送',
    },
    { label: '首次出现', value: fullTime(item.firstSeenAt) },
    { label: '最近出现', value: fullTime(item.lastSeenAt) },
    { label: '解决时间', value: item.resolvedAt ? fullTime(item.resolvedAt) : '-' },
  ];
}
function extraPayload(detail: NonNullable<DetailItem>) {
  const item = detail.item as Task | AuditLog | Alert;
  const payload =
    detail.type === 'tasks'
      ? (item as Task).payload
      : detail.type === 'audit'
        ? (item as AuditLog).metadata
        : (item as Alert).metadata;
  const parsed = parsePayload(payload);
  if (!parsed) return null;
  return {
    title:
      detail.type === 'tasks' ? '任务载荷' : detail.type === 'audit' ? '审计元数据' : '告警元数据',
...
function filterItems<T>(items: T[], query: string, selector: (item: T) => string) {
...
function taskProgress(task: Task) {
  const payload = parsePayload(task.payload) as
    ...
    | undefined;
  if (!payload?.totalAgents) return payload?.message || '-';
...
function statusTone(status: string): 'green' | 'red' | 'yellow' | 'blue' | 'gray' {
  if (['completed', 'resolved'].includes(status)) return 'green';
  if (status === 'failed') return 'red';
  if (['queued', 'active'].includes(status)) return 'yellow';
...
function alertLevelColor(level: string) {
  return { info: '#93c5fd', warning: '#fcd34d', critical: '#fca5a5' }[level] ?? 'var(--kvm-text)';
}
function labelResource(type: string) {
  return (
    { agent: 'Agent', host: '宿主机', virtual_machine: '虚拟机', system: '系统', snapshot: '快照' }[
      type
    ] ?? type
  );
}
function labelNotificationStatus(item: Alert) {
  return item.notificationSentAt ? '已触达' : '待发送';
}
function labelChannel(id: string) {
  return (
    {
      webhook: 'Webhook',
      email: '邮件',
...
function labelDeliveryStatus(status: string) {
  return { pending: '待发送', sent: '已发送', failed: '失败' }[status] ?? status;
}
function deliveryTone(status: string): 'green' | 'red' | 'yellow' | 'blue' | 'gray' {
...
const taskExportColumns: ExportColumn<Task>[] = [
  ...
  { id: 'status', header: '状态', value: item => labelStatus(item.status) },
  {
    id: 'target',
    header: '目标',
    value: item => `${item.targetType || '-'}/${item.targetId || '-'}`,
  },
  ...
  { id: 'createdAt', header: '创建时间', value: item => fullTime(item.created_at) },
  {
    id: 'finishedAt',
    header: '完成时间',
    value: item => (item.finished_at ? fullTime(item.finished_at) : '-'),
  },
];
...
const auditExportColumns: ExportColumn<AuditLog>[] = [
  ...
  { id: 'userId', header: '用户 ID', value: item => item.userId || '-' },
  {
    id: 'resource',
    header: '资源',
    value: item => `${item.resourceType}/${item.resourceId || '-'}`,
  },
...
const alertExportColumns: ExportColumn<Alert>[] = [
  ...
  { id: 'message', header: '消息', value: item => item.message },
  {
    id: 'source',
    header: '来源',
    value: item => `${labelResource(item.sourceType)}/${item.sourceId}`,
  },
