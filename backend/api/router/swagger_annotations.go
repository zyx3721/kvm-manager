...
// @Summary 获取通知媒介配置
// @Description 返回 Webhook、邮件、飞书、企业微信、钉钉及其应用通知媒介配置。平台内告警通过右上角通知中心展示。配置可包含 problemTemplate、recoveryTemplate、sendRecovery，Webhook 可额外包含 webhookProblemPayload、webhookRecoveryPayload。邮件 password、机器人 secret、应用 appSecret/secret 不返回明文，已配置时返回 hasPassword、hasSecret 或 hasAppSecret 标记。需要通知配置查看或管理权限。
// @Tags settings
// @Produce json
// @Security BearerAuth
...
// @Produce json
// @Security BearerAuth
// @Param id path string true "通知媒介 ID：webhook、email、lark、lark_app、wechat、wechat_app、dingtalk、dingtalk_app"
...
// @Summary 企业微信直连回调
// @Description 企业微信授权后的回调地址：state 用途为 login 时按绑定关系定位用户并签发会话（未绑定的企微账号会被拒绝）；用途为 bind 时把扫码账号绑定到发起绑定的用户。结果通过 URL fragment 302 回前端 /auth/callback 页面，失败时携带 error 与 message。
// @Tags auth
// @Produce json
...
// @Summary 解除企业微信绑定
// @Description 解除当前登录用户的企业微信账号绑定并写审计，未绑定时同样返回成功。返回 status 与被解绑的企微账号（未绑定为 -）。
// @Tags auth
// @Produce json
// @Security BearerAuth
// @Success 200 {object} wecomUnbindResponse
// @Failure 401 {object} errorResponse
// @Failure 500 {object} errorResponse
...
// @Summary 获取基础配置
// @Description 返回网站名称、登录页名称、控制台品牌名称、控制台品牌副标题、图标、安全时效、资源阈值、Agent 判定和告警通知策略配置。需要基础配置查看或管理权限。
// @Tags settings
// @Produce json
// @Security BearerAuth
// @Success 200 {object} domain.SystemBaseConfig
...
// @Summary 更新基础配置
// @Description 更新网站名称、登录/注册界面名称、控制台品牌名称、控制台品牌副标题、图标、安全时效、资源阈值、Agent 判定和告警通知策略配置。图标支持站内路径或图片 Data URL，最大 512KB。
// @Tags settings
// @Accept json
// @Produce json
// @Security BearerAuth
