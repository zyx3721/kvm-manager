...
- 发送验证码前需要输入验证邮箱，并选择已启用找回密码用途的媒介。
- 验证邮箱必须与用户名对应的用户配置邮箱一致。
- 邮件媒介发送到账号配置邮箱，Webhook 和机器人媒介直接发送到对应外部系统。
- 重置验证码 10 分钟内有效。
...
- 引入 KVM Agent 管理链路。
- 增加 Redis 运行态缓存、SSE 实时刷新和后端定时刷新。
- 调整数据持久化边界：宿主机、虚拟机、快照不再落数据库资源表，只保留运行态缓存。
- 增加 Agent 离线判定和活跃告警。
- 增加系统配置页面、通知媒介入口和账号修改密码能力。
- 增加忘记密码流程，支持通过已启用的邮件、飞书、企业微信和钉钉媒介发送重置验证码。
- 增加单 VM 监控弹窗、宿主机趋势入口、快照管理和任务 / 审计 / 告警页面。
